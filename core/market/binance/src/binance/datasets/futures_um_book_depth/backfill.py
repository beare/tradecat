...
def _relpath_daily_zip(symbol: str, d: date) -> str:
    sym = symbol.upper()
    return f"downloads/futures/um/daily/bookDepth/{sym}/{sym}-bookDepth-{d:%Y-%m-%d}.zip"


def _relpath_monthly_zip(symbol: str, month: str) -> str:
    sym = symbol.upper()
    return f"downloads/futures/um/monthly/bookDepth/{sym}/{sym}-bookDepth-{month}.zip"
    ...
    *,
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            """
            SELECT COUNT(*)
            FROM market.binance_futures_um_book_depth
            WHERE venue_id = %s
              AND instrument_id = %s
              AND timestamp >= %s
              AND timestamp < %s
    ...
    if import_writer:
        batch_id = import_writer.start_batch(
            ImportBatchSpec(
                ...
                note="binance_archive futures/um bookDepth backfill",
                meta={
                    "dataset": dataset,
