...
from binance.sources.binance_api.adapters.ccxt import load_symbols
...
LINEAGE_RESOURCE_ID = "facts/market/futures_um_metrics_snapshot_5m"
...
class MetricsCollector:
    ...
    def __init__(self, workers: int = 8):
        ...
        self._lineage_run_id = f"binance_data_metrics_{datetime.now(UTC).strftime('%Y%m%dT%H%M%S%fZ')}"
    ...
    def save(self, rows: list[dict]) -> int:
        ...
        try:
            ...
            emit_lineage_event(
                ...
                meta={"item_count": int(n), "source": "binance_api"},
            )
            return int(n)
            ...
            emit_lineage_event(
                ...
                status="failed",
                occurred_at=datetime.now(UTC),
                meta={"item_count": int(len(rows)), "error_text": str(exc)},
            )
            raise
