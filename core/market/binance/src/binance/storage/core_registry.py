"""market.shared_* 维表解析与注册（最小可用、可幂等）。

目标（说人话）：
- 事实表使用 (venue_id, instrument_id) 作为短主键，但采集侧天然拿到的是 (exchange, symbol)；
- 这里提供一个“最小且可重复执行”的映射层：
  - 确保 market.shared_venue 存在对应 venue_code
  - 确保 market.shared_instrument 存在对应 instrument
  - 确保 market.shared_symbol_map 存在 (venue_id, symbol) -> instrument_id 的当前映射

约束与取舍：
- 不引入额外 schema/table；只使用既有 market.shared_*（见 008_multi_market_core_and_storage.sql）
...
def _infer_instrument_type(symbol: str, *, product: str) -> str:
    """用最小规则推断 instrument_type（只用于 market.shared_instrument 的描述字段）。"""
...
class CoreRegistry:
    ...
    def _qualify_venue_code(self, *, venue_code: str, product: str) -> str:
        ...
        为什么要这样做（说人话）：
        - market.shared_symbol_map 的主键空间是 (venue_id, symbol, effective_from)，并且我们要加一条硬约束：
        ...
        - 若历史运行库曾把 futures_um 写在 venue_code=binance 下，需要先做一次性迁移：
          market.shared_venue: binance -> binance_futures_um（保持 venue_id 不变）
        """
    ...
    def _lock_symbol_mapping(self, *, venue_code: str, symbol: str, cursor: psycopg.Cursor) -> None:
        ...
        - market.shared_instrument 没有天然唯一键（除 PK），因此并发下必须显式加锁，才能保证“先查再建”语义稳定。
        """
    ...
    def get_or_create_venue_id(
        ...
        *,
        ...
        sql = """
        INSERT INTO market.shared_venue (venue_code, venue_name, timezone)
        VALUES (%s, %s, %s)
    ...
    def get_or_create_instrument_id(
        ...
        *,
        ...
        try:
            ...
            cur.execute(
                """
                INSERT INTO market.shared_instrument (asset_class, instrument_type, base_currency, quote_currency, meta)
                VALUES (
                  'crypto',
                  %s,
                  %s,
            ...
            cur.execute(
                """
                INSERT INTO market.shared_symbol_map (venue_id, symbol, instrument_id, effective_from, effective_to, meta)
                VALUES (
                  %s,
                  %s,
                  %s,
                  ...
                  NULL,
                  jsonb_build_object('source','auto','created_by','hf')
                )
                """,
        ...
        self._instrument_id_cache[cache_key] = instrument_id
        logger.info(
            "market.shared_symbol_map 新增映射: venue=%s(%d) symbol=%s -> instrument_id=%d",
            venue_code,
            venue_id,
            sym,
            instrument_id,
        ...
        *,
        ...
        try:
            ...
            if base_code == "binance" and prod == "futures_um" and qualified_venue_code != base_code:
                cur.execute("SELECT 1 FROM market.shared_venue WHERE venue_code = %s LIMIT 1", (base_code,))
                base_exists = cur.fetchone() is not None
                cur.execute("SELECT 1 FROM market.shared_venue WHERE venue_code = %s LIMIT 1", (qualified_venue_code,))
                qualified_exists = cur.fetchone() is not None
                if base_exists and not qualified_exists:
                    raise RuntimeError(
                        "检测到旧运行库 market.shared_venue(venue_code='binance') 仍存在，但采集已要求 futures_um 使用 "
