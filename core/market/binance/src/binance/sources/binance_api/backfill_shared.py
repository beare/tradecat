...
from assets.common.contracts.db_contracts import (
    market_candles_table_name as _market_candles_table_name,
    market_futures_um_metrics_snapshot_5m_table_name as _market_futures_um_metrics_snapshot_5m_table_name,
)

from binance.sources.binance_api.adapters.ccxt import fetch_ohlcv, load_symbols, to_rows
from binance.sources.binance_api.adapters.metrics import Timer, metrics
from binance.sources.binance_api.adapters.rate_limiter import acquire, parse_ban, release, set_ban
from binance.sources.binance_api.adapters.timescale import TimescaleAdapter
from binance.sources.binance_api.config import INTERVAL_TO_MS, settings
from binance.storage.ingest_meta import IngestMetaWriter
...
def completed_utc_day_window(lookback_days: int) -> tuple[date, date]:
    end = utc_today() - timedelta(days=1)
    start = end - timedelta(days=max(int(lookback_days), 0))
    return start, end


def _gap_window_ms(gap: "GapInfo") -> tuple[int, int]:
    start_ts = datetime.combine(gap.date, datetime.min.time(), tzinfo=UTC)
    end_ts = start_ts + timedelta(days=1)
    return int(start_ts.timestamp() * 1000), int(end_ts.timestamp() * 1000)


def _flatten_gap_keys(gaps: dict[str, list["GapInfo"]]) -> set[tuple[str, date]]:
    return {(symbol, gap.date) for symbol, sym_gaps in gaps.items() for gap in sym_gaps}


@dataclass
class GapGovernanceSummary:
    detected: int = 0
    opened: int = 0
    reopened: int = 0
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            ...
            "reopened": int(self.reopened),
            "skipped_ignored": int(self.skipped_ignored),
...
class GapGovernance:
    """LF gap 治理：冷却、ignored/reopen、补后关闭。"""
    ...
    def _writer(self):
        ...
        with self._ts.connection() as conn:
            yield IngestMetaWriter(conn)

    def filter_attemptable_gaps(
        self,
    ...
    ) -> tuple[dict[str, list["GapInfo"]], GapGovernanceSummary]:
        exchange_name = exchange or settings.db_exchange
        current = now or datetime.now(UTC)
        filtered: dict[str, list[GapInfo]] = {}
        summary = GapGovernanceSummary(detected=sum(len(items) for items in gaps.values()))

        with self._writer() as writer:
            for symbol, sym_gaps in gaps.items():
                for gap in sym_gaps:
                    start_ms, end_ms = _gap_window_ms(gap)
                    existing = writer.get_gap(
                    ...
                    if existing is None:
                        ...
                        summary.opened += 1
                        filtered.setdefault(symbol, []).append(gap)
                        continue

                    if existing.status == "ignored" and existing.detected_at is not None:
                        age_seconds = int((current - existing.detected_at.astimezone(UTC)).total_seconds())
                        if age_seconds < self._cooldown_seconds:
                            summary.skipped_ignored += 1
                            continue

                    if existing.status in {"ignored", "closed"}:
                        writer.reopen_gap(
                            existing.gap_id,
                            reason=f"{reason} | reopen_after={self._cooldown_seconds}s",
                        )
                        summary.reopened += 1
                    filtered.setdefault(symbol, []).append(gap)
    ...
    def finalize_attempt(
        self,
        attempted: dict[str, list["GapInfo"]],
        remaining: dict[str, list["GapInfo"]],
        *,
        ignore_reason: str,
        exchange: str | None = None,
    ) -> GapGovernanceSummary:
        exchange_name = exchange or settings.db_exchange
        summary = GapGovernanceSummary(remaining=sum(len(items) for items in remaining.values()))
        remaining_keys = _flatten_gap_keys(remaining)

        with self._writer() as writer:
            for symbol, sym_gaps in attempted.items():
                for gap in sym_gaps:
                    start_ms, end_ms = _gap_window_ms(gap)
                    existing = writer.get_gap(
                        exchange=exchange_name,
                    ...
                    if key in remaining_keys:
                        writer.ignore_gap(
                            existing.gap_id,
                            reason=f"{ignore_reason} | cooldown={self._cooldown_seconds}s",
                        )
                        summary.ignored += 1
                    elif existing.status != "closed":
                        writer.close_gap(existing.gap_id)
                        summary.closed += 1
        return summary
...
class GapScanner:
    ...
    def scan_klines(
        ...
        table = f"{self._ts.schema}.{_market_candles_table_name(interval)}"
        sql = f"""
    ...
    def scan_metrics(
        ...
        table = f"{self._ts.schema}.{_market_futures_um_metrics_snapshot_5m_table_name()}"
        sql = f"""
...
class ZipBackfiller:
    ...
    def _download_kline_month(self, symbol: str, month: str, dates: list[date], interval: str) -> int:
        ...
        sym = symbol.upper()
        total = 0
        current_month = utc_today().strftime("%Y-%m")
...
class DataBackfiller:
    ...
    def __init__(self, lookback_days: int = 10, workers: int = 8, threshold: float = 0.95):
        ...
        self._gap_cooldown_seconds = 3600
    ...
    def run_klines(self, symbols: Sequence[str] | None = None, interval: str = "1m") -> dict[str, int]:
        ...
        with Timer("last_backfill_duration"):
            ...
            start, end = completed_utc_day_window(self.lookback_days)
            cooldown_seconds = getattr(self, "_gap_cooldown_seconds", 3600)
            governance = GapGovernance(
                self._ts,
                "futures.um.candles_1m",
            ...
            attemptable, governance_open = governance.filter_attemptable_gaps(
                gaps,
                reason=f"lf_backfill_detected window={start}..{end}",
            )
            if not attemptable:
                logger.info("K 线缺口均处于 cooldown，跳过本轮补齐 | %s", governance_open.to_dict())
                result = {"scanned": len(symbols), "gaps": total_gaps, "filled": 0, "remaining": total_gaps}
                result.update(governance_open.to_dict())
                return result

            # 2. ZIP 补齐 (优先)
            filled = self._zip.fill_kline_gaps(attemptable, interval)

            # 3. 复检 + REST 补齐剩余
            remaining = self._scanner.scan_klines(list(attemptable.keys()), start, end, interval, self.threshold)
            if remaining:
                logger.info("复检: 仍有 %d 个缺口，尝试 REST 补齐", sum(len(g) for g in remaining.values()))
                filled += self._rest.fill_gaps(remaining, interval)

            # 4. 最终复检
            final = self._scanner.scan_klines(list(attemptable.keys()), start, end, interval, self.threshold)
            final_gaps = sum(len(g) for g in final.values()) if final else 0
            governance_close = governance.finalize_attempt(
                attemptable,
                final,
                ignore_reason=f"lf_backfill_unfillable window={start}..{end}",
            )

            metrics.inc("gaps_filled", filled)
            logger.info(
                "K 线补齐完成: 填充 %d 条, 剩余缺口 %d | governance=%s | %s",
                filled,
                final_gaps,
                {**governance_open.to_dict(), **governance_close.to_dict()},
                metrics,
            )
            result = {"scanned": len(symbols), "gaps": total_gaps, "filled": filled, "remaining": final_gaps}
            result.update(governance_open.to_dict())
            result.update(governance_close.to_dict())
            return result
    ...
    def run_metrics(self, symbols: Sequence[str] | None = None) -> dict[str, int]:
        ...
        governance = GapGovernance(
            self._ts,
            "futures.um.metrics_snapshot_5m",
            cooldown_seconds=cooldown_seconds,
        )
        ...
        attemptable, governance_open = governance.filter_attemptable_gaps(
            gaps,
            reason=f"lf_metrics_detected window={start}..{end}",
        )
        if not attemptable:
            logger.info("Metrics 缺口均处于 cooldown，跳过本轮补齐 | %s", governance_open.to_dict())
            result = {"scanned": len(symbols), "gaps": total_gaps, "filled": 0, "remaining": total_gaps}
            result.update(governance_open.to_dict())
            return result

        # 2. ZIP 补齐
        filled = self._zip.fill_metrics_gaps(attemptable)

        # 3. 复检 + REST 补齐剩余
        remaining = self._scanner.scan_metrics(list(attemptable.keys()), start, end, self.threshold)
        if remaining:
        ...
        # 4. 最终复检
        final = self._scanner.scan_metrics(list(attemptable.keys()), start, end, self.threshold)
        final_gaps = sum(len(g) for g in final.values()) if final else 0
        governance_close = governance.finalize_attempt(
            attemptable,
            final,
            ignore_reason=f"lf_metrics_unfillable window={start}..{end}",
        )

        logger.info(
            "Metrics 补齐完成: 填充 %d 条, 剩余缺口 %d | governance=%s",
            filled,
            final_gaps,
            {**governance_open.to_dict(), **governance_close.to_dict()},
        )
...
class GapFiller:
    ...
    def ensure_continuity(self, symbols: Sequence[str]) -> None:
        # lookback 是分钟，转换为天数 (向上取整)；只扫描“已结束的 UTC 自然日”。
        lookback_days = max(1, (self._lookback + 1439) // 1440)
        start, end = completed_utc_day_window(lookback_days)
...
def compute_lookback(mode: str, days: int, start_date: date | None = None) -> int:
    ...
    if mode == "all":
        if start_date:
            delta = (utc_today() - start_date).days
            return max(delta, 1)
        return 3650  # 默认 10 年
...
def main() -> None:
    ...
    try:
        if args.scan_only:
            # 仅扫描
            symbols = symbols or load_symbols(settings.ccxt_exchange)
            start, end = completed_utc_day_window(lookback)
