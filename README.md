...
交易猫行情派生数据，AI分析数据，预测市场数据在线终端，免费API服务：https://docs.google.com/spreadsheets/d/1q-2sXGsFYsKf3nV5u5golTVrLH5sfc0doiWwz_kavE4/edit?usp=sharing
...
# 端口：默认 5433（单集群：LF/HF 同库同端口），见 assets/config/.env.example
vim assets/config/.env

# 3) 启动核心服务（ai + signal + query + telegram + trading）
./scripts/start.sh start
./scripts/start.sh status
```

> 说明：顶层 `./scripts/start.sh` 默认管理 `plugins/ai`、`plugins/signal`、`core/query`、`plugins/telegram`、`plugins/trading`（`plugins/ai` 为子模块，仅做就绪检查，无独立进程；并保证 `core/query` 先于 telegram/sheets 启动；兼容旧名：api-service）。
> **重要**：从 2026-03 起，消费层（Telegram/Sheets/可视化）不再允许直连数据库，统一通过 **Query Service（`core/query`，`/api/v1`）** 读取数据；因此在运行 Telegram/Sheets 时 **不要停掉 `core/query`**。
> 例外：`plugins/ai` 当前被定义为研究型 / 离线分析插件，允许只读直连 `facts_data` / `derived_data`；它不是新的统一读出口。
> Binance 数据源栈：`core/market/binance/`（LF/HF 是运行分组，不再是子工程目录；不在默认启动链路）。
> 可选服务需手动启动：
> - `cd plugins/sheets && ./scripts/start.sh start`（Google Sheets 公共看板同步，默认 daemon）
...
- **数据库说明**（重要，按仓库现状）：
  - 默认单实例/单端口：`FACTS_DATABASE_URL=postgresql://.../facts_data`、`DERIVED_DATABASE_URL=postgresql://.../derived_data`
  - 兼容旧服务：`DATABASE_URL` 默认仍指向 `facts_data`
  - 若你坚持拆分 LF/HF：请把 `BINANCE_HF_DATABASE_URL` 指向另一实例/端口，并同步脚本/文档/服务配置
  - 若你在私有环境使用其它端口：请全局统一端口与脚本/命令；仓库示例以 5433 为准。<!-- TODO: 若仓库正式迁移到其它端口，请补“统一替换列表与执行顺序” -->
- 核心字段：
  - `FACTS_DATABASE_URL`（事实库；默认 `localhost:5433/facts_data`）
  - `DERIVED_DATABASE_URL`（派生库；默认 `localhost:5433/derived_data`）
  - `DATABASE_URL`（兼容旧服务，默认仍指向 `facts_data`）
  - `assets/contracts/resources.v1.yaml` / `probes.v1.yaml` / `bindings.v1.yaml` 共同构成**数据库对象命名与语义绑定**的唯一真相源
  - `assets/catalog/*` 只是导出 projection；运行时代码不要再从这里或 `.env` 决定已收口对象名
  - 运行时代码不要再裸写 `market.*` / `alternative.*` / `indicator_snapshots.*` / `signal_runtime.*`，统一通过 central contracts + `assets/common/contracts/db_contracts.py` 读取；`.env.example` 只保留连接/开关与少量未迁入 central contracts 的动态对象名
  ...
  - `QUERY_MARKET_TABLE_EXISTS_TTL_SEC`（可选：`facts_data.market.*` 表存在性检查缓存 TTL 秒数；默认 30）
...
```bash
# 安装依赖
cd core/market/binance && .venv/bin/python -m pip install pandas psycopg2-binary huggingface_hub

# 默认下载 Main4 数据集（BTC/ETH/BNB/SOL，415MB）
cd core/market/binance && .venv/bin/python scripts/capture_ccxt_pro_um_book_data.py

# 或指定币种
cd core/market/binance && .venv/bin/python scripts/capture_ccxt_pro_um_book_data.py --symbols BTCUSDT,ETHUSDT,BNBUSDT
```
...
> 端口说明：本文与仓库脚本示例按 `assets/config/.env.example` 默认端口（5433）编写；如你改动端口（或拆分 LF/HF 双集群），请同步所有示例命令与脚本配置。

## 🔍 补充检查（2026-01-23）

- **端口选择**：`assets/config/.env.example` 默认单端口 5433；仓库脚本示例亦以此为准。若你自行改动端口（或拆分双集群），请全局统一。
...
| 依赖 | 版本 | 说明 |
|:---|:---|:---|
| Python | 3.12+ | CI 使用 3.12；各服务 `core/**/pyproject.toml` / `plugins/**/pyproject.toml` 声明 `requires-python >=3.12`（`scripts/init.sh` 的最低检查为 3.10） |
...
# 或单独初始化某个服务
./scripts/init.sh hf  # 兼容旧名：hf-service
```
...
### 🔄 多市场数据采集（🟡预览/规划中）
- 说明：当前仓库核心链路聚焦加密货币；A股/美股/宏观/OpenBB 等属于 markets-service 预览方向（本仓库 `core/` 与 `plugins/` 下未内置实现，仅保留配置/文档占位）。
...
graph TD
    ...
    subgraph DS["📦 Binance Stack（LF 运行组）<br><small>Python, asyncio, ccxt, cryptofeed</small>"]
    ...
    PG_IND[("🗄️ indicator_snapshots.*<br>PostgreSQL 指标库")]
    TR_IND --> PG_IND
    ...
    subgraph ORD["💹 order-service（🟡预览：本仓库未在 core/plugins 下内置）<br><small>Python, ccxt, cryptofeed</small>"]
...
| 服务 | 端口 | 职责 | 技术栈 |
|:---|:---:|:---|:---|
| **binance-stack（LF）** | - | 加密货币 K线采集、期货指标采集、历史数据回填 | Python, asyncio, ccxt, cryptofeed |
...
| **predict-service** | - | 预测市场信号（Polymarket/Kalshi/Opinion，现归属 consumption 层） | Node.js + Python utilities |
| **nofx-dev** | - | Agentic Trading OS（预览/外部工程镜像，非核心链路） | Go, React, TypeScript |
| **markets-service（🟡预览：仅保留配置与文档，未在 core/plugins 下内置）** | - | 全市场数据采集（美股/A股/宏观/衍生品定价） | yfinance, akshare, fredapi, QuantLib |
| **order-service（🟡预览：仅保留配置与文档，未在 core/plugins 下内置）** | - | 交易执行、Avellaneda-Stoikov 做市 | Python, ccxt, cryptofeed |
| **TimescaleDB（单库）** | 5433 | LF+HF（K线/指标 + 原子事实；`DATABASE_URL`/`BINANCE_HF_DATABASE_URL` 默认同库同端口） | PostgreSQL 16 + TimescaleDB |
...
graph LR
    subgraph 数据采集
        A["🌐 币安 WebSocket"] --> B["📦 Binance Stack（LF）"]
    end
    ...
    subgraph 指标计算
        ...
        D --> E2[("🗄️ indicator_snapshots.*<br>PostgreSQL 指标库")]
    end
...
│
├── 📂 core/                        # 事实数据层（采集/落库/读出口）
│   ├── 📂 market/                  # 行情事实采集（写 TimescaleDB）
│   │   └── 📂 binance/             # 数据源：Binance
│   │       ├── 📂 data/            # 低频/分时采集（兼容链路，非默认启用）
│   │       └── 📂 hf/  # Binance 官方历史归档对齐采集（高频原子事实）
│   │
│   ├── 📂 alternative/             # 另类事实采集（事件/非结构化事实）
│   │   ├── 📂 api/
│   │   ├── 📂 discord/
│   │   ├── 📂 hyper/
│   │   ├── 📂 investing/
...
│
├── 📂 plugins/                     # 消费/派生插件层（只读 core/query；职责分类见 plugins/README.md）
│   ├── 📂 trading/                 # 派生：指标计算（写入 PG: indicator_snapshots.*）
│   ├── 📂 signal/                  # 派生：信号检测（规则引擎）
│   ├── 📂 ai/                      # AI：就绪检查/离线分析入口（非常驻进程）
│   ├── 📂 telegram/                # 导出：Telegram Bot + 事件/信号推送
│   ├── 📂 sheets/                  # 导出：Google Sheets 公共看板同步（可选）
│   ├── 📂 llm-gateway/             # 网关：nofx 外部提示词组装/决策网关（OpenAI compatible）
│   ├── 📂 predict/                 # 其它：预测市场信号（Polymarket/Kalshi/Opinion）
│   ├── 📂 fate/                    # 其它：命理/排盘（独立微服务）
│   ├── 📂 vis/                     # 其它：可视化渲染（可选）
│   └── 📂 nofx-dev/                # 其它：外部工程镜像（非核心链路）
│
...
```bash
# Binance Stack（LF 运行组；兼容旧名：data-service）
...
cd plugins/trading  # 或 plugins/telegram
...
```bash
# 查看所有相关进程
ps aux | grep -E "binance\.runtime\.lf_|trading-service|telegram|signal-service|src\.simple_scheduler|main\.py"
...
<details>
<summary><strong>点击展开👉 指标库（PG: indicator_snapshots）查询</strong></summary>

```bash
# 初始化 DDL（仅需一次；会创建 indicator_snapshots schema + 38 张表）
PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d derived_data \
    -f assets/database/db/schema/021_indicator_snapshots_sqlite_parity.sql
...
> 说明：当前指标库已统一为 PostgreSQL（`indicator_snapshots.*`）。
> 历史 SQLite 指标库若仍存在，仅用于迁移期对账/回放，不再作为当前运行真相。
