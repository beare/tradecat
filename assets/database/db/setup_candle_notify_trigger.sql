-- 创建 PG NOTIFY 触发器：当 futures_um_candles_1m 有数据变更时发送通知
-- 用于 K线合成器事件驱动模式

-- 1. 创建通知函数
CREATE OR REPLACE FUNCTION market.notify_binance_futures_um_candles_1m_update()
RETURNS TRIGGER AS $$
DECLARE
    payload JSON;
BEGIN
...
-- 2. 删除旧触发器（如果存在）
DROP TRIGGER IF EXISTS candle_1m_notify_trigger ON market.binance_futures_um_candles_1m;

-- 3. 创建触发器：在 INSERT 或 UPDATE 后触发
CREATE TRIGGER candle_1m_notify_trigger
AFTER INSERT OR UPDATE ON market.binance_futures_um_candles_1m
FOR EACH ROW
EXECUTE FUNCTION market.notify_binance_futures_um_candles_1m_update();
...
FROM pg_trigger 
WHERE tgrelid = 'market.binance_futures_um_candles_1m'::regclass
...
-- CREATE OR REPLACE FUNCTION market.notify_binance_futures_um_candles_1m_closed()
-- RETURNS TRIGGER AS $$
-- BEGIN
