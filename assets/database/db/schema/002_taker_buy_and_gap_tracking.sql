...
--
-- 使用说明：
-- - 执行本脚本前需已运行 `001_timescaledb.sql`（创建 market.binance_futures_um_* 基表）。
...
DROP FUNCTION IF EXISTS market.upsert_candle_1m(
    TEXT,
    TEXT,
    TIMESTAMPTZ,
    NUMERIC,
...
CREATE OR REPLACE FUNCTION market.upsert_candle_1m(
    p_exchange      TEXT,
    p_symbol        TEXT,
...
AS $$
BEGIN
    INSERT INTO market.binance_futures_um_candles_1m AS t (
...
CREATE TABLE IF NOT EXISTS market.binance_missing_intervals (
...
ALTER TABLE market.binance_missing_intervals
...
CREATE INDEX IF NOT EXISTS idx_missing_intervals_status_detected_at
    ON market.binance_missing_intervals (status, detected_at DESC);

CREATE INDEX IF NOT EXISTS idx_missing_intervals_exchange_symbol_interval
    ON market.binance_missing_intervals (exchange, symbol, "interval");
