...
-- 视图名：market.binance_futures_metrics_{interval}_last
-- start_offset = NULL 表示刷新全部可用数据

SET search_path TO market, public;

CREATE OR REPLACE FUNCTION market._创建指标连续聚合(
    p_view_name       TEXT,
...
BEGIN
    ...
    IF NOT view_exists THEN
        ...
        EXECUTE format($fmt$
            ...
            FROM market.binance_futures_um_metrics_snapshot_5m
            GROUP BY 1,2
            WITH NO DATA;
    ...
    -- 索引
    EXECUTE format(
        'CREATE INDEX IF NOT EXISTS %I ON market.%I (bucket, symbol);',
...
BEGIN
    ...
    LOOP
        PERFORM market._创建指标连续聚合(cfg.view_name, cfg.bucket_interval, cfg.expected_points, cfg.origin, cfg.end_offset, cfg.schedule_interval);
    END LOOP;
END$$;
