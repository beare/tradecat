-- Binance Futures Metrics 5m（当前真相：market.binance_futures_um_metrics_snapshot_5m）

CREATE SCHEMA IF NOT EXISTS market;

CREATE TABLE IF NOT EXISTS market.binance_futures_um_metrics_snapshot_5m (
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE c.conrelid = 'market.binance_futures_um_metrics_snapshot_5m'::regclass
          AND c.conname = 'uq_metrics_5m'
    ) THEN
        ALTER TABLE market.binance_futures_um_metrics_snapshot_5m
...
CREATE INDEX IF NOT EXISTS idx_metrics_5m_symbol_time
    ON market.binance_futures_um_metrics_snapshot_5m(symbol, create_time DESC);
CREATE INDEX IF NOT EXISTS idx_metrics_5m_exchange_symbol_time
    ON market.binance_futures_um_metrics_snapshot_5m(exchange, symbol, create_time DESC);
...
-- DO $$
-- BEGIN
--     PERFORM add_retention_policy('market.binance_futures_um_metrics_snapshot_5m', INTERVAL '730 days');
-- EXCEPTION WHEN duplicate_object THEN NULL;
-- END$$;
