...
from assets.common.contracts.db_contracts import (
    market_futures_um_candles_1m_table,
    market_futures_um_metrics_snapshot_5m_table,
)
...
def import_candles(conn, filepath: Path, symbols: list, days: int = None):
    ...
    cursor = conn.cursor()
    candles_table = market_futures_um_candles_1m_table()
    candles_table_name = candles_table.split(".", 1)[1]

    # 检查表是否存在
    cursor.execute("""
        SELECT EXISTS (
            SELECT FROM information_schema.tables 
            WHERE table_schema = 'market' AND table_name = %s
        )
    """, (candles_table_name,))
    if not cursor.fetchone()[0]:
        print(f"  ✗ 表 {candles_table} 不存在，请先导入 schema")
        return
...
def import_metrics(conn, filepath: Path, symbols: list, days: int = None):
    ...
    cursor = conn.cursor()
    metrics_table = market_futures_um_metrics_snapshot_5m_table()
    metrics_table_name = metrics_table.split(".", 1)[1]
    ...
    """, (metrics_table_name,))
    if not cursor.fetchone()[0]:
        print(f"  ✗ 表 {metrics_table} 不存在，跳过")
        return
