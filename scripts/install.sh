...
if command -v psql &>/dev/null; then
    ...
    echo -e "  ${YELLOW}⚠️ 请确保 PostgreSQL/TimescaleDB 已运行并导入当前 schema:${NC}"
    echo -e "     # 事实库"
    echo -e "     psql -h localhost -p 5433 -U opentd -d facts_data -f assets/database/db/stacks/facts.sql"
    echo -e ""
    echo -e "     # 派生库"
    echo -e "     psql -h localhost -p 5433 -U opentd -d derived_data -f assets/database/db/stacks/derived.sql"
else
...
echo "     # 事实库"
echo "     psql -h localhost -p 5433 -U opentd -d facts_data -f assets/database/db/stacks/facts.sql"
echo ""
echo "     # 派生库"
echo "     psql -h localhost -p 5433 -U opentd -d derived_data -f assets/database/db/stacks/derived.sql"
echo ""
echo "  3. 启动服务:"
echo "     ./scripts/start.sh start"
echo ""
echo "  4. 或单独启动:"
echo "     cd core/market/binance && make plan"
echo "     cd plugins/trading && ./scripts/start.sh start"
echo "     cd plugins/telegram && ./scripts/start.sh start"
