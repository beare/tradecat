...
SRC_ROOT = SERVICE_PATHS.src_root
PROJECT_ROOT = SERVICE_PATHS.service_root
REPO_ROOT = SERVICE_PATHS.repo_root
ASSETS_DIR = SERVICE_PATHS.assets_dir
ANIMATION_DIR = SERVICE_PATHS.animation_dir
LOCALE_STORE = SERVICE_PATHS.locale_store

ensure_runtime_sys_path()
ENV_FILE = load_shared_env()
...
# Binance API 已废弃，数据由 lf-service 采集
...
class TradeCatBot:
    ...
    async def update_cache_lightweight(self):
        ...
        # 数据由 lf-service 采集，此处仅更新本地计算的缓存
        critical_tasks = [
            ('active_symbols_cache', lambda: self.get_active_symbols(force_refresh=True)),
        ]
