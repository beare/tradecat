...
from ..db.reader import shared_indicator_pg_conn, shared_pg_conn
from ..config import config, get_market_futures_metrics_table


# ==================== 写入结果 ====================

def write_results(all_results: Dict[str, list]):
    """写入指标结果（PG: indicator_snapshots schema）"""
...
def update_market_share():
    ...
    try:
        ...
        with shared_pg_conn() as conn:
            with conn.cursor() as cur:
                ...
                for interval in ["15m", "1h", "4h", "1d", "1w"]:
                    metrics_table = get_market_futures_metrics_table(interval)
                    cur.execute(f"""
                        SELECT SUM(sum_open_interest_value)
                        FROM {metrics_table}
                        WHERE bucket = (SELECT MAX(bucket) FROM {metrics_table})
                    """)
                    row = cur.fetchone()
                    if row and row[0]:
        ...
        # 2. 更新指标库（PG: indicator_snapshots）
...
def cleanup_futures_1m():
    """清理期货表的1m数据（期货无1m粒度）"""
    try:
        with shared_indicator_pg_conn() as pg_conn:
            with pg_conn.cursor() as cur:
                cur.execute(
                    sql.SQL('DELETE FROM {}.{} WHERE "周期"=%s').format(
                        sql.Identifier(config.indicator_pg_schema),
                        sql.Identifier("期货情绪聚合表.py"),
                    ),
                    ("1m",),
