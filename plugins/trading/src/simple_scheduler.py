...
from src.config import (
    ...
    get_market_ingest_offsets_table,
    load_shared_env,
)

load_shared_env()

TRADING_SERVICE_DIR = str(SERVICE_ROOT)
...
from src.db.reader import shared_indicator_pg_conn, shared_pg_conn
...
def _parse_tg_ts(v: object) -> datetime | None:
    """
    解析 indicator_snapshots.* 的 "数据时间"（text）为 UTC tz-aware datetime。
...
def _query_kline_priority(top_n: int = 30) -> set:
    ...
    try:
        with shared_pg_conn() as conn:
            candles_5m_table = get_market_candles_table("5m")
...
def _query_futures_priority(top_n: int = 30) -> set:
    ...
    result = set()
    futures_metrics_5m_table = get_market_futures_metrics_table("5m")
    try:
        with shared_pg_conn() as conn:
            with conn.cursor() as cur:
                ...
                """.format(table=futures_metrics_5m_table))
                rows = cur.fetchall()
...
def _load_all_symbols_from_db() -> list:
    ...
    ingest_offsets_table = get_market_ingest_offsets_table()
    try:
        with shared_pg_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                """.format(table=ingest_offsets_table))
                rows = cur.fetchall()
...
def get_source_latest(interval: str) -> datetime:
    ...
    table = get_market_candles_table(interval)
    try:
        with shared_pg_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(f"SELECT MAX(bucket_ts) as latest FROM {table}")
                row = cur.fetchone()
...
def get_futures_source_latest(interval: str) -> datetime:
    ...
    try:
        with shared_pg_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                table = get_market_futures_metrics_table(interval)
                if interval == "5m":
                    cur.execute(f"SELECT MAX(create_time) AS latest FROM {table}")
                else:
                    cur.execute(f"SELECT MAX(bucket) AS latest FROM {table}")
                row = cur.fetchone()
...
def get_indicator_latest(interval: str) -> datetime:
    """查询 PG 指标该周期最新数据时间（indicator_snapshots）"""
    try:
        with shared_indicator_pg_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    sql.SQL('SELECT MAX("数据时间") AS latest FROM {}.{} WHERE "周期"=%s').format(
                        sql.Identifier(config.indicator_pg_schema),
                        sql.Identifier("MACD柱状扫描器.py"),
                    ),
                    (interval,),
                )
                row = cur.fetchone()
        ...
        log(f"查询 {config.indicator_pg_schema} 指标 {interval} 最新时间失败: {e}")
        return None
...
def get_futures_indicator_latest(interval: str) -> datetime:
    ...
    try:
        with shared_indicator_pg_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    sql.SQL('SELECT MAX("数据时间") AS latest FROM {}.{} WHERE "周期"=%s AND "持仓金额" IS NOT NULL').format(
                        sql.Identifier(config.indicator_pg_schema),
                        sql.Identifier("期货情绪元数据.py"),
                    ),
                    (interval,),
                )
                meta = cur.fetchone()
        ...
        log(f"查询 {config.indicator_pg_schema} 期货指标 {interval} 最新时间失败: {e}")
        return None
