...
指标计算服务：从 TimescaleDB 读取 K线/期货数据，计算 38 个技术指标，并写入 PostgreSQL 指标库（`indicator_snapshots.*`）。
...
| `INDICATOR_PG_SCHEMA` | indicator_snapshots | PG 指标 schema |
| `MAX_WORKERS` | 4 | 并行线程数 |
...
```bash
DATABASE_URL=postgresql://user:password@localhost:5432/facts_data
...
TimescaleDB (candles_1m)
    ...
    Engine (并行计算 38 指标)
        ...
        ▼
    Indicator Store
      ├─ PG (indicator_snapshots.* legacy 多表)
      └─ PG (indicator_snapshots.snapshots 单表 JSONB；用于治理收敛)
        │
        ▼
...
指标结果写入 `DERIVED_DATABASE_URL` 指向的 PG 库（默认 `derived_data`）的 `indicator_snapshots` schema：

- legacy 布局：`indicator_snapshots.*`（每指标一表）
- snapshots 布局：`indicator_snapshots.snapshots`（单表 JSONB；推荐）

切换方式：设置 `INDICATOR_PG_LAYOUT=legacy|dual|snapshot`（见 `assets/config/.env.example`）。

初始化 `indicator_snapshots` DDL（仅需一次）：

```bash
PGPASSWORD=postgres psql 'postgresql://postgres:postgres@localhost:5433/derived_data' \
  -f assets/database/db/schema/021_indicator_snapshots_sqlite_parity.sql \
  -f assets/database/db/schema/026_indicator_snapshots_snapshots.sql
```
