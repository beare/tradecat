...
SYSTEMD_UNIT="tradecat-sheets.service"

systemd_available() {
    command -v systemctl >/dev/null 2>&1 || return 1
    systemctl --user cat "$SYSTEMD_UNIT" >/dev/null 2>&1 || return 1
    return 0
}

systemd_dispatch() {
    local action="${1:-status}"
    echo "ℹ️  检测到 systemd 用户服务：$SYSTEMD_UNIT（将由 systemctl 管理，避免重复 daemon）"
    case "$action" in
        start) systemctl --user start "$SYSTEMD_UNIT" ;;
        stop) systemctl --user stop "$SYSTEMD_UNIT" ;;
...
find_repo_root() {
    ...
    local p="$start"
    while [[ -n "$p" && "$p" != "/" ]]; do
        if [[ -d "$p/scripts" && -d "$p/assets/config" && ( -d "$p/core" || -d "$p/plugins" ) ]]; then
            echo "$p"
            return 0
        fi
        p="$(dirname "$p")"
...
if [[ -z "$PROJECT_ROOT" ]]; then
    echo "❌ 错误: 无法定位 PROJECT_ROOT（从 $SERVICE_DIR 向上未找到 scripts + assets/config + core/plugins）"
    exit 1
fi
...
start_svc() {
    ...
    nohup "$VENV_DIR/bin/python" -u -B -m src --daemon >> "$OUT_LOG" 2>&1 &
    local new_pid=$!
