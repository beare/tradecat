...
def export_symbol_query_sheet(*, symbol: str, lang: str = "zh_CN") -> SymbolQuerySheet:
    ...
    interval_s = _env_float("SHEETS_SYMBOL_TABS_INTERVAL_SECONDS")
    mode = (os.environ.get("SHEETS_WRITE_MODE", "") or "").strip().lower() or None
    meta_text = format_public_meta_row_text(
        ...
        mode=mode,
        schema="symbol_query_v2",
        extra_kv=[("币种", sym), ("说明", "结构化表格（非文本伪表格）")],
    )
