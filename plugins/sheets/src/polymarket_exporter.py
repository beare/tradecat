...
def _detect_polymarket_service_dir() -> Path | None:
    ...
    if repo_root is not None:
        candidates += [
            # 当前仓库布局（推荐）
            repo_root / "plugins" / "other" / "predict-service" / "services" / "polymarket",
            # 旧采集层布局（兼容）
...
def export_polymarket_stats_sheet(*, lang: str = "zh_CN") -> PolymarketStatsSheet:
    ...
    interval_s = _env_float("SHEETS_POLYMARKET_STATS_INTERVAL_SECONDS")
    ...
    try:
        ...
        meta_text = format_public_meta_row_text(
            dataset="Polymarket",
            exported_at_utc8=str(now),
            interval_seconds=interval_s,
            window="滚动24h",
            row_count=None,
            lang=str(lang or "zh_CN").strip() or "zh_CN",
            mode="auto",
            schema="polymarket_stats_v1",
            extra_kv=[("错误", f"{type(exc).__name__}:{exc}"[:200])],
        )
        values = [[meta_text]]
    ...
    meta_text = format_public_meta_row_text(
        ...
        row_count=None,
        lang=str(lang or "zh_CN").strip() or "zh_CN",
        mode=str(source_desc or "").strip() or None,
        schema="polymarket_stats_v1",
    )
