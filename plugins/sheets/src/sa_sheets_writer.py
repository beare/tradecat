...
def _borderless_enabled() -> bool:
    """
    是否启用“无边框”展示（全局开关）。

    语义：
    - gridProperties.hideGridlines = true（隐藏默认网格线）
    - 禁止渲染可见边框（例如字段组分隔线、卡片外框等）

    配置：
    - env: SHEETS_STYLE_BORDERLESS=1
    """
    raw = (os.environ.get("SHEETS_STYLE_BORDERLESS", "") or "").strip().lower()
    if not raw:
        return False
    return raw not in {"0", "false", "off", "no", "none"}


def _borderless_override() -> bool | None:
    """
    “无边框”开关的显式覆盖值（用于写入 sheetProperties）。

    - None：环境变量未设置 -> 不主动改写 hideGridlines（保持用户/历史状态）
    - True/False：环境变量已设置 -> 视为“希望强制覆盖”的运维意图
    """
    raw = (os.environ.get("SHEETS_STYLE_BORDERLESS", "") or "").strip().lower()
    if not raw:
        return None
...
def _looks_like_public_banner_text(v: Any) -> bool:
    """
    判断一个单元格是否“像广告位 banner”。
    - 新格式：首行=“广告位”，后续多行是链接/文案
    - 兼容旧格式：广告位（...）
    """
    s = str(v or "").strip()
    if not s:
    ...
    if first == "广告位":
        return True
    if first.startswith("广告位（") or first.startswith("广告位("):
        return True
    # 更宽松一点：允许“广告位xxx”这种变体
    if first.startswith("广告位"):
        return True
    return False


def _looks_like_public_meta_text(v: Any) -> bool:
    """
    判断一个单元格是否“像公开表元信息行”（A2）。
    约定：中文逗号分隔 key/value，且至少包含“数据源/导出时间”。
    """
    s = str(v or "").strip()
    if not s:
        return False
    # 允许未来把 key 做兼容（导出时间(UTC+8) -> 导出时间）
    return ("数据源" in s) and (("导出时间(UTC+8)" in s) or ("导出时间" in s))
...
class SaSheetsWriter:
    ...
    def __init__(
        ...
        *,
        ...
        self._tab_dashboard = _env_text("SHEETS_TAB_DASHBOARD", "加密货币看板")
        ...
        self._frozen_by_title: dict[str, tuple[int, int]] = {}
        ...
        # cache：公开表 banner 文案（避免每个 tab 反复读取）
        self._public_banner_text_cache: str | None = None
        self._public_banner_lines_cache: int | None = None
    ...
    def spreadsheet_id(self) -> str:
        return str(self._spreadsheet_id or "")
    ...
    def ensure_schema(self) -> None:
        ...
        if self._schema_mode == "minimal":
            ...
            # 兼容：旧工作簿 tab 命名迁移（例如历史英文名、宏观看板旧名）。
            # minimal schema 也要做这一步，否则会出现“同一内容被新建为新 tab”的体验问题。
            try:
        ...
        # 事实/索引层默认隐藏：公开工作簿/看板场景下不应暴露这些内部表
        for t in [
            self._tab_cards_index,
            self._tab_card_fields_eav,
            self._tab_card_rows,
            ...
            try:
                self._set_sheet_hidden(str(t), hidden=True)
            except Exception:
                pass
    ...
    def public_top_banner_text(self) -> tuple[str, int]:
        """
        返回“全表首行广告位”文案（单单元格多行文本）与行数。

        优先级：
        1) env: SHEETS_TOP_BANNER_TEXT / SHEETS_DASHBOARD_BANNER_TEXT
        2) 兜底：从现有工作簿中读取任意一个看起来像 banner 的 A1 作为“规范文案”

        设计目的：
        - 避免运维忘记配置 env 时，新建 tab 缺 banner
        - 让“工作簿本身”成为可恢复的单一真相源（banner 不依赖本机配置）
        """
        if self._public_banner_text_cache is not None and self._public_banner_lines_cache is not None:
            return str(self._public_banner_text_cache), int(self._public_banner_lines_cache)

        raw_env = _read_top_banner_raw(prefer_dashboard=False)
        text, lines = _format_dashboard_banner_text(str(raw_env or ""))
        if text:
            self._public_banner_text_cache = str(text)
            self._public_banner_lines_cache = int(lines or 0)
            return str(text), int(lines or 0)

        # env 未配置：从工作簿“现有内容”兜底
        try:
            self._refresh_sheet_map()
        except Exception:
            pass
        ...
        candidates: list[str] = []
        if self._tab_dashboard:
            candidates.append(str(self._tab_dashboard))
        try:
            for sh in self.list_sheet_properties():
                t = str(sh.get("title") or "").strip()
        ...
        for t in candidates:
            try:
                got = self.read_values_a1(title=t, a1_range="A1:A1")
                a1 = str(got[0][0] or "") if got and got[0] else ""
                if not _looks_like_public_banner_text(a1):
                    continue
                text2, lines2 = _format_dashboard_banner_text(a1)
                if text2:
                    self._public_banner_text_cache = str(text2)
                    self._public_banner_lines_cache = int(lines2 or 0)
                    return str(text2), int(lines2 or 0)
            except Exception:
                continue

        # 仍为空：不强行写入，交给调用方决定（通常意味着新建工作簿且未配置 banner）
        self._public_banner_text_cache = ""
        self._public_banner_lines_cache = 0
        return "", 0
    ...
    def ensure_sheet(self, *, title: str) -> None:
        self.ensure_schema()
        # 性能：高频写入场景下，避免每次都 get 一次 sheets 列表（会迅速耗尽 read quota）。
        # - 若 cache 已包含该 tab，则直接返回
        # - 仅在未知 title 时强制 refresh 一次
        if title in self._sheet_id_by_title:
            return
        self._refresh_sheet_map()
        if title in self._sheet_id_by_title:
    ...
    def reset_sheet_display(
        ...
        *,
        ...
        if compact:
            # 重要：Google Sheets API 不允许“把所有非冻结列/行都删光”。
            # 若当前 frozenColumnCount=2，而你直接把 columnCount 缩到 2（即便同一请求里把 frozenColumnCount 改成 1），
            # 也会报错：`it is not possible to delete all non-frozen columns`。
            #
            # 因此这里做两步更新：
            # 1) 先把 frozenRow/ColCount 调整到目标值（不缩 grid）
            # 2) 再缩 rowCount/columnCount（此时不会“删光非冻结”）
            self._refresh_sheet_map()
            cur_rows, cur_cols = self._grid_by_title.get(title, (0, 0))
            cur_fr, cur_fc = self._frozen_by_title.get(title, (0, 0))

            target_rows = max(int(frozen_row_count) + 1, 2)
            target_cols = max(int(_col_to_index(col_r_u)), 1)

            # 防呆：冻结列必须 < 总列数，否则会导致“无非冻结列”而被 API 拒绝
            frozen_column_count = min(int(frozen_column_count), max(int(target_cols) - 1, 0))
            frozen_row_count = min(int(frozen_row_count), max(int(target_rows) - 1, 0))

            borderless_override = _borderless_override()
            gp_freeze: dict[str, Any] = {
            ...
            if borderless_override is not None:
                gp_freeze["hideGridlines"] = bool(borderless_override)
                fields_freeze.append("gridProperties.hideGridlines")
                gp_final["hideGridlines"] = bool(borderless_override)
                fields_final.append("gridProperties.hideGridlines")

            need_freeze_first = False
            if int(target_cols) < int(cur_cols) and int(target_cols) <= int(cur_fc):
                need_freeze_first = True
            if int(target_rows) < int(cur_rows) and int(target_rows) <= int(cur_fr):
                need_freeze_first = True

            if need_freeze_first:
                self._exec(
                    self._sheets.spreadsheets().batchUpdate(
                        spreadsheetId=self._spreadsheet_id,
                        body={
                            "requests": [
                                {
                                    "updateSheetProperties": {
                                        "properties": {
                                            "sheetId": int(sh_id),
                                            "gridProperties": gp_freeze,
                                        },
                                        "fields": ",".join(fields_freeze),
                                    }
                                }
                            ]
                        },
                        ...
                        "requests": [
                            {
                                "updateSheetProperties": {
                                    "properties": {
                                        "sheetId": int(sh_id),
                                        "gridProperties": gp_final,
                                    },
                                    "fields": ",".join(fields_final),
                                }
                            }
                        ]
                    },
    ...
    def write_symbol_query_tab(self, *, tab_title: str, sheet: Any) -> dict[str, Any]:
        ...
        banner_text, banner_line_count = self.public_top_banner_text()
        ...
        # -------------------- style --------------------
        # 说明：
        # - 币种查询子表是“完全托管展示面”，历史上最容易踩坑的是：layout 变更后样式不全量刷新（冻结区错位）。
        # - 这里用 `style_signature` 做强约束：任何影响样式/冻结/目录的关键参数变化，都必须触发全量重刷。
        style_version = "symbol_table_v13"
        key_style_version = f"symtab.{tab_title}.style_version"
        key_style_signature = f"symtab.{tab_title}.style_signature"
        ...
        # 样式签名：一旦变化就强制全量刷新（避免“修了但某些 tab 仍残留旧样式”的多世界分裂）。
        # NOTE: 不把 n_rows/n_cols 直接放进 signature，避免因数据波动造成每轮全量重刷。
        compact_grid = (os.environ.get("SHEETS_SYMBOL_QUERY_COMPACT_GRID", "1") or "1").strip() != "0"
        style_sig = _sha256_hex(
            json.dumps(
                {
                    "kind": "symbol_query",
                    ...
                    "banner_line_count": int(banner_line_count or 0),
                    "directory_row_0": int(directory_row_0),
                    "header_row_0": int(header_row_0),
                    "frozen_cols": int(_symbol_query_frozen_cols()),
                    "placeholder": str(placeholder_style),
                },
                ensure_ascii=False,
                sort_keys=True,
            )
        ...
        # 样式覆盖范围：
        # - compact grid 开启时：优先按“当前数据行列”收敛（避免历史 styled_rows=800 锁死导致样式永不刷新）。
        # - compact grid 关闭时：按历史样式范围扩展（避免留白区域在 UI 中可见但未被规范化样式覆盖）。
        if compact_grid:
            target_rows = int(max(n_rows, 1))
            target_cols = int(max(n_cols, 6))
            need_style = (
                (meta.get(key_style_signature) or "") != style_sig
    ...
    def _write_polymarket_stats_tab_grid(
        ...
        *,
        ...
        if need_style:
            ...
            # gridlines：默认不改写（保持用户/历史状态）；若显式配置“无边框”，则按配置覆盖。
            borderless_override = _borderless_override()
            if borderless_override is not None:
                reqs.append(
                    {
                        "updateSheetProperties": {
                            "properties": {
                                "sheetId": int(sh_id),
                                "gridProperties": {"hideGridlines": bool(borderless_override)},
                            },
            ...
            borderless = bool(_borderless_enabled())
    ...
    def _write_polymarket_stats_tab_report(
        ...
        *,
        ...
        if need_style:
            ...
            borderless_override = _borderless_override()
            if borderless_override is not None:
                gp_last = reqs[-1]["updateSheetProperties"]["properties"]["gridProperties"]
                gp_last["hideGridlines"] = bool(borderless_override)
                reqs[-1]["updateSheetProperties"]["fields"] += ",gridProperties.hideGridlines"
        ...
        # gridlines：默认不隐藏（历史版本可能设置为隐藏）；启用“无边框”时隐藏。
        borderless_override = _borderless_override()
        if borderless_override is not None:
            try:
                self._exec(
    ...
    def _write_polymarket_stats_tabs_split_report(
        ...
        *,
        ...
        # 公开工作簿安全兜底：
        # - split 模式下我们不会覆盖写“源 tab”，会导致历史内容残留（包括错误信息/路径片段）
        # - 这里把源 tab 清空并写入“占位说明”，避免公开读场景泄露内部路径或造成困惑
        try:
            banner_text, _banner_line_count = self.public_top_banner_text()
            try:
                interval_s = float((os.environ.get("SHEETS_POLYMARKET_STATS_INTERVAL_SECONDS", "") or "").strip() or "0")
                if interval_s <= 0:
                    interval_s = None
            except Exception:
                interval_s = None
            meta_placeholder = format_public_meta_row_text(
                dataset="Polymarket",
                exported_at_utc8=_now_utc8_iso(),
                interval_seconds=interval_s,
                window="滚动24h",
                row_count=None,
                lang="zh_CN",
                mode="split",
                schema="polymarket_stats_source_v1",
                extra_kv=[("子表", "源tab（已拆分）")],
            )
            placeholder: list[list[Any]] = []
            if banner_text:
                placeholder.append([banner_text, "", ""])
            placeholder.append([meta_placeholder, "", ""])
            placeholder.append(["本 tab 仅占位：统计已拆分为 PolymarketTop15 / Polymarket时段分布 / Polymarket类别偏好", "", ""])
            ...
            self.reset_sheet_display(
                ...
                col_r="C",
                compact=True,
                frozen_row_count=min(len(placeholder), 3),
                frozen_column_count=0,
            )
            self.write_values_matrix(title=tab_title, values=placeholder, value_input_option="USER_ENTERED")
        except Exception:
            pass
    ...
    def write_dashboard_v5_main(self, *, payloads: list[dict[str, Any]], col_l: str, col_r: str) -> dict[str, Any]:
        """
        将“方案5（字段纵向 + 周期横向）”作为主看板写入到 `SHEETS_TAB_DASHBOARD`（默认：加密货币看板）。
        ...
        try:
            banner_text, _banner_line_count = self.public_top_banner_text()
            banner_rows = 1 if banner_text else 0
            frozen_rows = int(banner_rows) + 3  # 元信息 + 目录 + 表头 + (可选 banner)
            self._set_sheet_grid_properties(
                self._tab_dashboard,
    ...
    def write_dashboard_v5_meta_tab(self, *, payloads: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        # minimal schema：默认不创建“元信息层”隐藏 tab；如需保留可显式开启。
        default_enabled = "0" if self._schema_mode == "minimal" else "1"
        enabled = (os.environ.get("SHEETS_DASHBOARD_V5_META_ENABLED", default_enabled) or default_enabled).strip() != "0"
        if not enabled:
        ...
        try:
            self._set_sheet_grid_properties(title, row_count=n_rows, col_count=n_cols, frozen_row_count=1, frozen_column_count=0)
        except Exception:
            pass
    ...
    def write_dashboard_v5_data_tab(self, *, payloads: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        # minimal schema：默认只保留“看板 + 币种查询”，不应额外创建隐藏数据层 tab；
        # 如确有需求可显式设置 `SHEETS_DASHBOARD_V5_DATA_ENABLED=1` 重新开启。
        default_enabled = "0" if self._schema_mode == "minimal" else "1"
        enabled = (os.environ.get("SHEETS_DASHBOARD_V5_DATA_ENABLED", default_enabled) or default_enabled).strip() != "0"
        if not enabled:
            return {"ok": True, "skipped": True, "reason": "disabled"}
    ...
    def append_dashboard_v5_history_tab(self, *, payloads: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            self._set_sheet_grid_properties(title, row_count=end_row, col_count=n_cols, frozen_row_count=1, frozen_column_count=0)
        except Exception:
            pass
    ...
    def _write_v5_field_rows_period_columns_sheet(
        ...
        *,
        ...
        meta_rows = 1
        dir_rows = 1
        meta_row_1 = int(banner_rows) + 1  # 1-based
        dir_row_1 = int(banner_rows) + int(meta_rows) + 1  # 1-based
        header_row_1 = int(banner_rows) + int(meta_rows) + int(dir_rows) + 1  # 1-based
        ...
        meta_text = format_public_meta_row_text(
            dataset=str(sheet_title),
            exported_at_utc8=str(export_ts_utc or _now_utc8_iso()),
            interval_seconds=None,
            window=None,
            row_count=int(len(render_payloads)),
            ...
            schema="dashboard_v5_main",
        )
        data.append({"range": f"{sheet_title}!{col_l}{meta_row_1}:{col_r_eff}{meta_row_1}", "values": [pad_row([meta_text])]})
        ...
        try:
            ...
            prev_dir_rows = 0
        try:
            prev_meta_rows = int(str(meta.get(key_meta_rows) or "0").strip() or "0")
        except Exception:
            prev_meta_rows = 0
        try:
        ...
        full_style = (
            (meta.get(key_style_version) or "") != style_version
            or (meta.get(key_style_placeholder) or "") != placeholder_style
            or int(prev_meta_rows) != int(meta_rows)
        ...
        # 元信息行样式（始终覆盖，避免旧格式残留）
        if int(meta_rows) > 0:
            meta_r0 = int(banner_rows)
            meta_r1 = int(banner_rows) + int(meta_rows)
            reqs.append(
                {
                    "repeatCell": {
                        "range": rrange(r0=int(meta_r0), r1=int(meta_r1), c0=col_l0, c1=col_r1),
                        "cell": {
                            "userEnteredFormat": {
                                "backgroundColor": _rgb(0.96, 0.97, 0.98),
                                "textFormat": {"bold": True},
            ...
            # 视觉合并：遮蔽 meta 行内部竖线（冻结列开启时无法真 merge）
            reqs.append(
                {
                    "updateBorders": {
                        "range": rrange(r0=int(meta_r0), r1=int(meta_r0) + 1, c0=col_l0, c1=col_r1),
                        "innerVertical": {"style": "SOLID", "width": 1, "color": _rgb(0.96, 0.97, 0.98)},
                    }
                }
            )
        ...
        # 目录区样式（始终覆盖，避免旧格式残留）
        if int(dir_rows) > 0:
            dir_r0 = int(banner_rows) + int(meta_rows)
            dir_r1 = int(banner_rows) + int(meta_rows) + int(dir_rows)
            reqs.append(
                {
                    "repeatCell": {
                ...
                self._meta_set(
                    {
                        ...
                        key_meta_rows: str(int(meta_rows)),
        ...
        try:
            ...
            self._set_sheet_grid_properties(
                ...
                col_count=int(used_cols),
                frozen_row_count=int(banner_rows) + int(meta_rows) + int(dir_rows) + 1,
                frozen_column_count=int(frozen_cols),
            )
        except Exception:
            pass
    ...
    def reset_dashboard(self, *, col_l: str, col_r: str, compact: bool = False) -> dict[str, Any]:
        ...
        if compact:
            ...
            if borderless_override is not None:
                gp["hideGridlines"] = bool(borderless_override)
    ...
    def _migrate_legacy_tabs(self) -> None:
        ...
        """
        legacy_to_target = {
            # 宏观展示页旧命名：迁移为中文（不以数字开头）
            "01_DASHBOARD": "宏观大宗看板",
            # 主看板旧命名：迁移为更明确的中文名
            "看板": self._tab_dashboard,
    ...
    def _refresh_sheet_map(self) -> None:
        ...
        for sh in ss.get("sheets", []):
            ...
            grid[title] = (rc, cc)
            try:
                fr = int(gp.get("frozenRowCount") or 0)
                fc = int(gp.get("frozenColumnCount") or 0)
            except Exception:
                fr, fc = 0, 0
            frozen[title] = (fr, fc)
        self._sheet_id_by_title = out
        self._grid_by_title = grid
        self._frozen_by_title = frozen
    ...
    def get_sheet_grid_fragment(
        ...
        *,
        ...
        """
        读取指定 sheet 的一小段 grid 数据（includeGridData=true），用于审计/验收。

        说明：
        - 只读：不会写入任何数据/样式
        - range 会自动对 title 做 A1 quote（支持中文/空格）
        """
        self._refresh_sheet_map()
        if title not in self._sheet_id_by_title:
            raise RuntimeError(f"missing_sheet:{title}")

        rng = f"{self._a1_quote_title(title)}!{str(a1_range or '').strip() or 'A1:A1'}"
        ss = self._exec(
            self._sheets.spreadsheets().get(
                spreadsheetId=self._spreadsheet_id,
                ranges=[rng],
                includeGridData=True,
                fields=str(fields or "").strip() or "sheets(properties(title),data(rowData(values(userEnteredFormat))))",
            ),
            is_write=False,
        )
    ...
    def get_sheets_grid_fragments(
        ...
        *,
        ...
        """
        一次性读取多个 sheet 的小范围 grid 数据（includeGridData=true），用于审计/验收。

        设计目的：
        - 降低 read request 次数，减少 429（尤其是“连续跑多个审计命令”的场景）
        - 每个 title 仅请求一个 range（同一 sheet 多 range 暂不支持）
        """
        wanted = [str(t or "").strip() for t in (titles or []) if str(t or "").strip()]
        if not wanted:
            return []
        ...
        ranges: list[str] = []
        for t in wanted:
            if t not in self._sheet_id_by_title:
                continue
            ranges.append(f"{self._a1_quote_title(t)}!{str(a1_range or '').strip() or 'A1:A1'}")
        if not ranges:
            return []
        ...
        ss = self._exec(
            self._sheets.spreadsheets().get(
                ...
                or "sheets(properties(title),data(rowData(values(userEnteredFormat))))",
            ),
            is_write=False,
        )
        out = ss.get("sheets") or []
    ...
    def _set_sheet_grid_properties(
        ...
        *,
        ...
        # -------------------- invariants（结构性 400 防线） --------------------
        # Google Sheets 不允许 “删除所有非冻结行/列”：
        # - rowCount 必须 >= frozenRowCount + 1
        # - columnCount 必须 >= frozenColumnCount + 1
        #
        # 这是一个结构性不变量：必须在唯一 choke point 收敛，否则调用点稍有遗漏就会在运行时爆 400，
        # 表现为“导出失败/看起来服务挂了”。
        row0, col0, fr0, fc0 = row_count, col_count, frozen_row_count, frozen_column_count
        try:
        ...
        try:
            ...
            col_count = None
        try:
            frozen_row_count = int(frozen_row_count) if frozen_row_count is not None else None
        except Exception:
            frozen_row_count = None
        try:
            frozen_column_count = int(frozen_column_count) if frozen_column_count is not None else None
        except Exception:
        ...
        if frozen_row_count is not None:
            frozen_row_count = max(int(frozen_row_count), 0)
        if frozen_column_count is not None:
            frozen_column_count = max(int(frozen_column_count), 0)

        # 如果只设置冻结行/列（不传 row/col），则用当前网格大小做 clamp，避免 freeze==rowCount 导致结构性错误。
        # NOTE: `_grid_by_title` 由 `_refresh_sheet_map` 维护；缺失时按需 refresh 一次（只读）。
        cur_rows, cur_cols = self._grid_by_title.get(str(title), (0, 0))
        cur_rows = int(cur_rows or 0)
        cur_cols = int(cur_cols or 0)
        if (cur_rows <= 0 or cur_cols <= 0) and (
            (row_count is None and frozen_row_count is not None) or (col_count is None and frozen_column_count is not None)
        ):
            try:
                self._refresh_sheet_map()
                cur_rows, cur_cols = self._grid_by_title.get(str(title), (0, 0))
        ...
        if frozen_row_count is not None and row_count is None and cur_rows > 0:
            frozen_row_count = min(int(frozen_row_count), max(int(cur_rows) - 1, 0))
        if frozen_column_count is not None and col_count is None and cur_cols > 0:
            frozen_column_count = min(int(frozen_column_count), max(int(cur_cols) - 1, 0))

        # 如果同时设置 row/col 与 frozen，则优先“收敛冻结数”，避免通过扩大 row/col 引入额外空白行/列。
        # - 约束：frozen < rowCount / colCount（至少保留 1 行/列非冻结）
        if row_count is not None and frozen_row_count is not None:
            frozen_row_count = min(int(frozen_row_count), max(int(row_count) - 1, 0))
        if col_count is not None and frozen_column_count is not None:
            frozen_column_count = min(int(frozen_column_count), max(int(col_count) - 1, 0))

        if _debug_enabled() and (row0, col0, fr0, fc0) != (row_count, col_count, frozen_row_count, frozen_column_count):
            _debug_print(
                "sheets.grid_clamp "
                f"title={title} "
                f"row={row0}->{row_count} col={col0}->{col_count} "
                f"frozen_row={fr0}->{frozen_row_count} frozen_col={fc0}->{frozen_column_count}"
            )
        ...
        # 网格线开关：默认不改写（保持用户/历史状态）；若显式配置“无边框”，则按配置覆盖。
        borderless_override = _borderless_override()
        if borderless_override is not None:
            gp["hideGridlines"] = bool(borderless_override)
            fields.append("gridProperties.hideGridlines")
        ...
        if row_count is not None:
            gp["rowCount"] = int(row_count)
            fields.append("gridProperties.rowCount")
        if col_count is not None:
            gp["columnCount"] = int(col_count)
            fields.append("gridProperties.columnCount")
        if frozen_row_count is not None:
            gp["frozenRowCount"] = int(frozen_row_count)
            fields.append("gridProperties.frozenRowCount")
        if frozen_column_count is not None:
            gp["frozenColumnCount"] = int(frozen_column_count)
    ...
    # 运维友好：显式暴露“本地 meta”（不走 Sheets API），给高频任务做幂等/节流
    def local_meta_get(self) -> dict[str, str]:
        return self._local_meta_get()

    def local_meta_set(self, kv: dict[str, str], *, clear_prefixes: list[str] | None = None) -> None:
        self._local_meta_set(kv, clear_prefixes=clear_prefixes)

    # ==================== lightweight generic helpers（给宏观展示/脚本用） ====================
    def sheet_id(self, title: str) -> int:
        """
        返回指定 sheet 的 gid（sheetId）。
        - 若不存在会先 ensure_sheet
        """
        self.ensure_sheet(title=title)
        sheet_id = self._sheet_id_by_title.get(title)
    ...
    def batch_update(self, *, requests: list[dict[str, Any]]) -> None:
        """
        透传 Sheets batchUpdate（写请求），用于少量样式/合并等操作。
        """
        if not requests:
            return
        self._exec(
    ...
    def clear_conditional_format_rules(self, *, title: str) -> int:
        """
        删除指定 sheet 的全部 conditional format 规则。
        ...
        for sh in ss.get("sheets", []) or []:
            ...
            if int(props.get("sheetId") or -1) != sh_id:
                continue
            cond_cnt = int(len((sh or {}).get("conditionalFormats") or []))
            break

        if cond_cnt <= 0:
            return 0

        reqs = [{"deleteConditionalFormatRule": {"sheetId": int(sh_id), "index": 0}} for _ in range(int(cond_cnt))]
        self.batch_update(requests=reqs)
        return int(cond_cnt)

    def write_values_matrix(self, *, title: str, values: list[list[Any]], value_input_option: str = "RAW") -> None:
        """
        从 A1 起覆盖写一个 values 矩阵（RAW/USER_ENTERED）。
        - 自动扩容 grid，避免超范围写入报错
        - 不清样式；如需清空请先 reset_sheet_display
        """
        if not values:
            return
        self.ensure_sheet(title=title)
        ...
        self._ensure_grid_size(title, min_rows=int(n_rows), min_cols=int(n_cols))
        col_r = _index_to_col(int(n_cols))
        ...
        def do_update() -> None:
            self._exec(
                ...
                .update(
                    spreadsheetId=self._spreadsheet_id,
                    range=f"{q}!A1:{col_r}{n_rows}",
                    valueInputOption=str(value_input_option or "RAW"),
                    body={"values": values},
                ),
                is_write=True,
            )
        ...
        try:
            ...
            # 自愈：高频写入场景下，我们为了省 read quota 会缓存 sheet map。
            # 但若 tab 在后台被 prune/重命名/删除，缓存会变脏，导致 values.update 报：
            # - HttpError 400: Unable to parse range: '<title>'!A1
            status = int(getattr(getattr(exc, "resp", None), "status", 0) or 0)
            msg = str(exc)
            if status == 400 and "Unable to parse range" in msg:
                try:
                    self._refresh_sheet_map()
                except Exception:
                    pass
    ...
    def clear_sheet_banding(self, *, title: str) -> int:
        """
        删除指定 sheet 的所有 bandedRanges（交替底色）。

        说明：
        - 只影响 banding，不影响单元格内容/边框/条件格式
        - 用于“zebra 灰白交替”反复刷新时避免累积多条 banding
        """
        t = str(title or "").strip()
        if not t:
            return 0
        ...
        for sh in ss.get("sheets", []) or []:
            props = (sh or {}).get("properties") or {}
            if str(props.get("title") or "").strip() != t:
                continue
            for br in (sh or {}).get("bandedRanges", []) or []:
                try:
                    bid = int((br or {}).get("bandedRangeId") or 0)
                except Exception:
                    bid = 0
                if bid > 0:
                    ids.append(int(bid))
        ...
        reqs = [{"deleteBanding": {"bandedRangeId": int(bid)}} for bid in ids]
        self.batch_update(requests=reqs)
        return int(len(ids))

    def apply_row_banding_zebra(
        self,
        *,
        title: str,
        start_row_index: int,
        ...
        start_col_index: int,
        end_col_index: int,
        first_band_color: dict[str, Any],
        second_band_color: dict[str, Any],
        clear_existing: bool = True,
    ) -> dict[str, Any]:
        """
        对指定范围应用“灰白交替”行底色（banding）。

        注意：
        - banding 不是边框，属于背景色规则；对外展示更稳定（不依赖公式/locale）。
        - 建议 start_row_index 从“数据首行”开始，避免覆盖 header/bannner/meta 的底色。
        """
        t = str(title or "").strip()
        if not t:
        ...
        if sid is None:
            raise RuntimeError(f"missing_sheet:{t}")

        # clamp range：避免传入负数或越界导致 400
        rc, cc = self._grid_by_title.get(t, (0, 0))
        sr = max(int(start_row_index), 0)
        er = max(int(end_row_index), 0)
        sc = max(int(start_col_index), 0)
        ec = max(int(end_col_index), 0)
        if int(rc) > 0:
            sr = min(sr, int(rc))
            er = min(er, int(rc))
        if int(cc) > 0:
            ...
            ec = min(ec, int(cc))
        if er <= sr or ec <= sc:
            return {"ok": True, "skipped": True, "reason": "empty_range"}

        deleted = 0
        if clear_existing:
            try:
                deleted = int(self.clear_sheet_banding(title=t))
            except Exception:
                deleted = 0
        ...
        reqs.append(
            {
                "addBanding": {
                    "bandedRange": {
                        "range": {
                            "sheetId": int(sid),
                            "startRowIndex": int(sr),
                            "endRowIndex": int(er),
                            "startColumnIndex": int(sc),
                            "endColumnIndex": int(ec),
                        },
                        "rowProperties": {
                            "firstBandColor": dict(first_band_color),
                            "secondBandColor": dict(second_band_color),
                        },
                    }
                }
            }
        ...
        self.batch_update(requests=reqs)
        return {"ok": True, "deleted": int(deleted), "added": 1}

    def style_workbook_borderless_all(self, *, include_hidden: bool = True) -> dict[str, Any]:
        """
        一次性把当前工作簿所有 GRID tabs 设置为“无边框”：
        - gridProperties.hideGridlines = true（隐藏默认网格线）
        - 清除边框（top/bottom/left/right/innerHorizontal/innerVertical = NONE）

        注意：
        - 该操作会覆盖用户手动设置的边框样式（不可自动恢复）。
        - 建议配合 env `SHEETS_STYLE_BORDERLESS=1`，避免后续刷新又把网格线/边框改回去。
        """
        ss = self._exec(
            self._sheets.spreadsheets().get(
                spreadsheetId=self._spreadsheet_id,
                fields="sheets.properties(sheetId,title,sheetType,hidden,gridProperties(rowCount,columnCount,hideGridlines))",
            ),
            is_write=False,
        )
        ...
        def add_clear_borders(sheet_id: int, *, row_count: int, col_count: int) -> None:
            if int(row_count) <= 0 or int(col_count) <= 0:
                return
            reqs.append(
                {
                    "updateBorders": {
        ...
        for sh in ss.get("sheets", []) or []:
            ...
            sheet_type = str(props.get("sheetType") or "GRID").strip().upper()
            hidden = bool(props.get("hidden") or False)
            gp = props.get("gridProperties") or {}
            try:
                row_count = int(gp.get("rowCount") or 0)
                col_count = int(gp.get("columnCount") or 0)
            except Exception:
                row_count, col_count = 0, 0

            if sheet_type != "GRID":
                skipped.append({"title": title, "reason": f"skip_sheet_type:{sheet_type}"})
                continue
            if hidden and (not include_hidden):
                skipped.append({"title": title, "reason": "skip_hidden"})
                continue
            if sheet_id <= 0:
                skipped.append({"title": title, "reason": "skip_missing_sheet_id"})
                continue
            if row_count <= 0 or col_count <= 0:
                skipped.append({"title": title, "reason": f"skip_empty_grid:{row_count}x{col_count}"})
                continue

            reqs.append(
                {
                    "updateSheetProperties": {
                        "properties": {"sheetId": int(sheet_id), "gridProperties": {"hideGridlines": True}},
                        "fields": "gridProperties.hideGridlines",
                    }
                }
            )
            add_clear_borders(int(sheet_id), row_count=int(row_count), col_count=int(col_count))
            styled.append(title or str(sheet_id))

        # Sheets API 的 batchUpdate 单次 requests 数量有限；这里按 chunk 发送，避免 400/413。
        # 经验值：每个 tab 两个 request；chunk=400 足够覆盖 200+ tabs。
        chunk = 400
        sent = 0
        for i in range(0, len(reqs), int(chunk)):
            part = reqs[i : i + int(chunk)]
            if part:
                self.batch_update(requests=part)
                sent += len(part)
        ...
        return {
            "ok": True,
            "styled_tabs": len(styled),
            "skipped_tabs": len(skipped),
            "requests": int(sent),
            "tabs": styled,
            "skipped": skipped,
        }

    def fix_workbook_public_top_rows_all(
        self,
        *,
        include_hidden: bool = False,
        min_frozen_rows: int = 3,
        dry_run: bool = False,
    ) -> dict[str, Any]:
        """
        运维治理：统一工作簿所有 tab 的“Top 行”结构与样式。

        约定（公开表统一结构）：
        - Row1(A1): 广告位 banner（单单元格多行文本）
        - Row2(A2): 元信息 meta（单单元格单行文本）
        - Row3+: 原有表头/数据（整体下移，不破坏公式引用）

        行为：
        - 若缺 banner：在最顶部插入 1 行，再写入 A1
        - 若 banner 文案不一致：覆盖 A1 为“规范 banner”
        - 若缺 meta：在 banner 下插入 1 行，再写入 A2（只在缺失时创建；不重复插入）
        - 统一 Row1/Row2 样式与行高（视觉合并：遮蔽内部竖线）
        - 冻结行数至少 min_frozen_rows（默认 3：banner+meta+header）
        """
        banner_text, banner_line_count = self.public_top_banner_text()
        if not banner_text:
            return {"ok": False, "error": "missing_banner_text（未配置 env，且工作簿内也未找到 banner 可兜底）"}
        ...
        export_lang = (os.environ.get("SHEETS_EXPORT_LANG", "zh_CN") or "zh_CN").strip() or "zh_CN"
        write_mode = (os.environ.get("SHEETS_WRITE_MODE", "") or "").strip().lower() or "sa"
        exported_at = _now_utc8_iso()

        def guess_schema(title: str) -> str:
            t = str(title or "").strip()
            if not t:
                return "generic_v1"
            if t == "API":
            ...
            if t == "实时新闻":
                return "news_v2"
            if t == "加密货币看板":
                return "dashboard_v5_main"
            if t == "宏观大宗看板":
                return "macro_dashboard_v1"
            if t.startswith("币种查询_"):
                return "symbol_query_v2"
            if t.startswith("Polymarket"):
                return "polymarket_stats_v1"
            return "generic_v1"

        ss = self._exec(
            self._sheets.spreadsheets().get(
                spreadsheetId=self._spreadsheet_id,
                fields="sheets.properties(sheetId,title,index,hidden,sheetType,gridProperties(columnCount,frozenRowCount,frozenColumnCount))",
            ),
            is_write=False,
        )
        ...
        for sh in ss.get("sheets", []) or []:
            ...
            try:
                ...
                frozen_rows = 0
            try:
                frozen_cols = int(gp.get("frozenColumnCount") or 0)
            except Exception:
                frozen_cols = 0

            if not title or sheet_id <= 0:
                skipped.append({"title": title, "reason": "skip_missing_title_or_sheet_id"})
                continue
            if sheet_type != "GRID":
                skipped.append({"title": title, "reason": f"skip_sheet_type:{sheet_type}"})
                continue
            if hidden and (not include_hidden):
                ...
                continue
            if col_count <= 0:
                # 无法设定 repeatCell 的 endColumnIndex；跳过
                skipped.append({"title": title, "reason": f"skip_empty_grid_cols:{col_count}"})
                continue

            def read_a1_a2() -> tuple[str, str]:
                a1 = ""
                a2 = ""
                try:
                    got = self.read_values_a1(title=title, a1_range="A1:A2")
                    if got and got[0]:
                        a1 = str(got[0][0] or "")
            ...
            actions: dict[str, Any] = {"title": title, "gid": sheet_id, "hidden": hidden}
            ...
            try:
                ...
                # -------- banner --------
                if not _looks_like_public_banner_text(a1):
                    actions["insert_banner_row"] = True
                    if not dry_run:
                        self._exec(
                            self._sheets.spreadsheets().batchUpdate(
                                spreadsheetId=self._spreadsheet_id,
                ...
                # banner 文案统一（允许旧格式归一化）
                cur_banner_norm, _cur_lines = _format_dashboard_banner_text(a1)
                want_banner_norm, _want_lines = _format_dashboard_banner_text(banner_text)
                if want_banner_norm and (cur_banner_norm.strip() != want_banner_norm.strip()):
                    actions["update_banner_text"] = True
                    if not dry_run:
                        self._exec(
                            self._sheets.spreadsheets()
                            .values()
                            .update(
                                spreadsheetId=self._spreadsheet_id,
                                range=f"{self._a1_quote_title(title)}!A1",
                                valueInputOption="RAW",
                                body={"values": [[want_banner_norm]]},
                            ),
                            is_write=True,
                        )
                    a1, a2 = read_a1_a2()
                ...
                # -------- meta --------
                meta_ok = _looks_like_public_meta_text(a2)
                if not meta_ok:
                    meta_text = format_public_meta_row_text(
                        dataset=str(title),
                        exported_at_utc8=str(exported_at),
                        interval_seconds=None,
                        window=None,
                        row_count=None,
                        lang=str(export_lang),
                        mode=str(write_mode),
                        schema=str(guess_schema(title)),
                    )
                    # “像旧 meta，但缺关键字段” -> 原地覆写；否则插入一行再写
                    if "数据源" in str(a2 or ""):
                        actions["rewrite_meta_row"] = True
                        if not dry_run:
                            self._exec(
                                self._sheets.spreadsheets()
                                .values()
                                .update(
                                    spreadsheetId=self._spreadsheet_id,
                                    range=f"{self._a1_quote_title(title)}!A2",
                                    valueInputOption="RAW",
                                    body={"values": [[meta_text]]},
                                ),
                                is_write=True,
                            )
                    else:
                        actions["insert_meta_row"] = True
                        if not dry_run:
                            self._exec(
                                self._sheets.spreadsheets().batchUpdate(
                                    spreadsheetId=self._spreadsheet_id,
                ...
                # -------- style (banner+meta) --------
                actions["style_top_rows"] = True
                if not dry_run:
                    # 先 unmerge 顶部两行：避免历史合并残留导致 banner/meta 文案在 UI 里被截断。
                    # - 只处理前两行，避免误伤正文的合并（例如“类别列纵向合并”）。
                    top_cols = min(int(col_count), 120)
                    if top_cols > 0:
                        try:
                            self.batch_update(
                                requests=[
                    ...
                    # 清空 banner/meta 行的其它列：保证“单单元格”语义，且允许横向溢出绘制。
                    if int(top_cols) > 1:
                        try:
                            end_col = _index_to_col(int(top_cols))
                            blanks = [[""] * (int(top_cols) - 1), [""] * (int(top_cols) - 1)]
                            self._exec(
                                self._sheets.spreadsheets()
                                .values()
                                .update(
                                    spreadsheetId=self._spreadsheet_id,
                                    range=f"{self._a1_quote_title(title)}!B1:{end_col}2",
                                    valueInputOption="RAW",
                                    body={"values": blanks},
                                ),
                                is_write=True,
                    ...
                    # freeze rows: 至少 banner+meta+header
                    want_fr = max(int(frozen_rows), int(min_frozen_rows))
                    if want_fr != int(frozen_rows):
                        actions["update_frozen_rows"] = {"from": int(frozen_rows), "to": int(want_fr)}
                        reqs.append(
                            {
                                "updateSheetProperties": {
                                    "properties": {
                                        "sheetId": int(sheet_id),
                                        "gridProperties": {"frozenRowCount": int(want_fr), "frozenColumnCount": int(frozen_cols)},
                                    },
                ...
                errors.append({"title": title, "error": f"{type(exc).__name__}:{exc}"[:2000]})

        return {
            "ok": True,
            "dry_run": bool(dry_run),
            "include_hidden": bool(include_hidden),
            "min_frozen_rows": int(min_frozen_rows),
            "total": int(len(updated) + len(skipped) + len(errors)),
            "updated": int(len(updated)),
            "skipped": int(len(skipped)),
            "errors": int(len(errors)),
            "updated_tabs": updated,
            "skipped_tabs": skipped,
            "error_tabs": errors,
        }

    # ==================== public read/write helpers（用于 API tab 等） ====================
    @staticmethod
    def _a1_quote_title(title: str) -> str:
        """
        将 sheet title 规范为 A1 notation 可用的形式：
        - 包裹单引号
        - 内部单引号转义为两个单引号
        """
    ...
    def list_sheet_properties(self) -> list[dict[str, Any]]:
        """
        列出当前工作簿所有 sheet 的基础属性（用于 API 目录表等）。
        返回字段：gid/title/index/hidden
        """
        ss = self._exec(
            self._sheets.spreadsheets().get(
                spreadsheetId=self._spreadsheet_id,
        ...
        for s in ss.get("sheets") or []:
            prop = (s or {}).get("properties") or {}
            try:
                gid = int(prop.get("sheetId") or 0)
            except Exception:
                gid = 0
            title = str(prop.get("title") or "")
            try:
                index = int(prop.get("index") or 0)
            except Exception:
                index = 0
            hidden = bool(prop.get("hidden") or False)
            if not title or gid <= 0:
                continue
            out.append({"gid": gid, "title": title, "index": index, "hidden": hidden})
        out.sort(key=lambda x: (int(x.get("index") or 0), str(x.get("title") or "")))
        return out

    def _sheet_used_size(self, *, title: str, value_render_option: str = "FORMULA") -> tuple[int, int]:
        """
        读取整个 sheet 的 values（稀疏矩阵）并返回“最大有效行/列”。

        - 默认 `value_render_option=FORMULA`：可以把“有公式但结果为空”的单元格也算作有效区域，
          避免误裁剪导致公式被删除。
        - 注意：该方法只考虑“值/公式”，不会把“仅有样式/数据验证”的单元格算作有效区域。
        """
        rng = self._a1_quote_title(title)
        got = self._exec(
            self._sheets.spreadsheets()
            .values()
            .get(
                spreadsheetId=self._spreadsheet_id,
                range=rng,
                valueRenderOption=str(value_render_option or "FORMULA"),
            ),
            is_write=False,
        )
        ...
        if not isinstance(values, list):
            return 0, 0
        used_rows = int(len(values))
        used_cols = 0
        for r in values:
            if isinstance(r, list):
                used_cols = max(int(used_cols), int(len(r)))
        return int(used_rows), int(used_cols)

    def audit_grid_used_range_all(
        self,
        *,
        ...
        """
        审计所有 GRID tabs 的网格大小（gridProperties）与“最大有效行/列”（values.get 裁剪结果）。

        返回字段：
        - title/index/hidden/sheetType
        - gridRows/gridCols/frozenRows/frozenCols
        - usedRows/usedCols（按 value_render_option 计算）
        - wantRows/wantCols（考虑冻结行/列的不变量）
        - extraRows/extraCols/status/error
        """
        ss = self._exec(
            self._sheets.spreadsheets().get(
                spreadsheetId=self._spreadsheet_id,
        ...
        out: list[dict[str, Any]] = []
        for sh in ss.get("sheets", []) or []:
            prop = (sh or {}).get("properties") or {}
            title = str(prop.get("title") or "").strip()
            if not title:
                continue
            sheet_type = str(prop.get("sheetType") or "GRID").strip().upper()
            if sheet_type != "GRID":
                continue
            ...
            try:
                ...
                grid_rows, grid_cols, frozen_rows, frozen_cols = 0, 0, 0, 0

            err = ""
            try:
                used_rows, used_cols = self._sheet_used_size(title=title, value_render_option=value_render_option)
            except Exception as exc:
                used_rows, used_cols = -1, -1
                err = f"{type(exc).__name__}:{exc}"[:2000]

            want_rows = max(int(used_rows if used_rows >= 0 else 0), int(frozen_rows) + 1, 1)
            want_cols = max(int(used_cols if used_cols >= 0 else 0), int(frozen_cols) + 1, 1)

            extra_rows = int(grid_rows) - int(want_rows)
            extra_cols = int(grid_cols) - int(want_cols)
            status = "ok" if (extra_rows == 0 and extra_cols == 0) else ("err" if err else "bigger")
            ...
            out.append(
                {
                    ...
                    "frozenRows": int(frozen_rows),
                    "frozenCols": int(frozen_cols),
                    "usedRows": int(used_rows),
                    "usedCols": int(used_cols),
                    "wantRows": int(want_rows),
                    "wantCols": int(want_cols),
                    "extraRows": int(extra_rows),
                    "extraCols": int(extra_cols),
                    "status": status,
                    "error": err,
                }
            )
    ...
    def compact_grid_to_used_range_all(
        self,
        *,
        include_hidden: bool = True,
        value_render_option: str = "FORMULA",
        ...
        """
        将所有 GRID tabs 的网格裁剪到“最大有效行/列”（按 values.get + value_render_option 计算）。

        ⚠️ 风险：
        - 收缩 rowCount/columnCount 会“删除”网格外的单元格（等价于 UI 删除多余行/列）。
        - 本函数按“值/公式”判断有效范围；如果你在远处只放了样式/数据验证，可能会被裁掉。
        """
        audit = self.audit_grid_used_range_all(include_hidden=include_hidden, value_render_option=value_render_option)
        updated: list[dict[str, Any]] = []
        skipped: list[dict[str, Any]] = []
        errors: list[dict[str, Any]] = []
        ...
        for row in audit:
            ...
            if not title:
                continue
            if str(row.get("status") or "") == "err":
                errors.append({"title": title, "error": str(row.get("error") or "")})
                continue

            extra_rows = int(row.get("extraRows") or 0)
            extra_cols = int(row.get("extraCols") or 0)
            if extra_rows == 0 and extra_cols == 0:
                skipped.append({"title": title, "reason": "already_compact"})
                continue

            want_rows = int(row.get("wantRows") or 0)
            want_cols = int(row.get("wantCols") or 0)
            frozen_rows = int(row.get("frozenRows") or 0)
            frozen_cols = int(row.get("frozenCols") or 0)

            if dry_run:
                updated.append(
                    {
                        "title": title,
                        "grid": [int(row.get("gridRows") or 0), int(row.get("gridCols") or 0)],
                        "want": [want_rows, want_cols],
                    }
                )
                continue
    ...
    def read_values_a1(self, *, title: str, a1_range: str) -> list[list[Any]]:
        """
        读取指定 sheet 的 A1 范围 values（只读）。
        注意：返回为稀疏矩阵（Google values.get 默认裁剪末尾空行/空列）。
        """
        rng = f"{self._a1_quote_title(title)}!{a1_range}"
        got = self._exec(
            self._sheets.spreadsheets()
            .values()
            .get(
    ...
    def write_values_a1(
        ...
        *,
        ...
        """
        从指定 A1 起覆盖写 values（RAW/USER_ENTERED）。
        - 采用 values.update：幂等写（适合“快照/单行覆盖”类写入）
        - 不清样式；如需清空请先 reset_sheet_display 或 values.clear
        """
        if not values:
            return
        self.ensure_schema()
        ...
        # 自动扩容 grid：避免“默认 26 列”写入 60+ 列时报错
        try:
            start = str(a1_start or "A1").strip() or "A1"
            ...
            for r in values:
                if isinstance(r, list):
                    n_cols = max(n_cols, len(r))
            need_rows = max((start_row - 1) + int(n_rows), 1)
            need_cols = max((start_col - 1) + int(n_cols), 1)
            self._ensure_grid_size(title, min_rows=int(need_rows), min_cols=int(need_cols))
        except Exception:
            # 扩容失败不阻断；让底层 update 自己报错，便于定位真实原因
            pass
        ...
        self._exec(
            ...
            .update(
                spreadsheetId=self._spreadsheet_id,
                range=f"{q}!{str(a1_start or 'A1').strip() or 'A1'}",
                valueInputOption=str(value_input_option or "RAW"),
                body={"values": values},
            ),
            is_write=True,
    ...
    def write_api_table_values(self, *, api_title: str, rows: list[list[str]]) -> None:
        """
        将 API 目录表整体覆盖写入（A/B/C 三列）。
        - 先 clear A:C（不清样式），再从 A1 更新 values
        - 采用 values.update：幂等写（更易推断）
        """
        self.ensure_schema()
        self.ensure_sheet(title=api_title)

        # -------------------- top banner（全表首行广告位） --------------------
        # 要求：API tab 也必须有首行广告位，且风格/行高与其它 tab 完全一致。
        banner_text, banner_line_count = self.public_top_banner_text()
        banner_rows = 1 if banner_text else 0
        if banner_rows > 0:
            first_a = ""
            try:
                ...
                first_a = ""
            if first_a != str(banner_text or "").strip():
                rows = [[banner_text, "", ""]] + list(rows or [])

        # -------------------- top meta row（全表第二行元信息） --------------------
        # 需求：每个公开 tab 必须在 banner 下有一行元信息（单单元格），用于展示导出时间/刷新间隔等。
        meta_r0 = int(banner_rows)
        if len(rows) <= meta_r0:
            rows = list(rows or []) + [["", "", ""]]
        try:
            cur_meta = str((rows[int(meta_r0)][0] if rows[int(meta_r0)] else "") or "").strip()
        except Exception:
            cur_meta = ""
        if not cur_meta:
            rows[int(meta_r0)] = [
        ...
        # 规整：保证每行 3 列（A/B/C），避免 values.update 写入后出现“残留旧值”
        rows3: list[list[str]] = []
        for r in rows or []:
            rr = [("" if x is None else str(x)) for x in (r or [])]
            if len(rr) < 3:
                rr += [""] * (3 - len(rr))
            rows3.append(rr[:3])
        rows = rows3

        api_q = self._a1_quote_title(api_title)
        self._exec(
            self._sheets.spreadsheets()
            .values()
            .clear(
        ...
        # 智能裁剪：API tab 是完全托管展示面，允许把底部/右侧空白网格裁剪掉（节省 cells 配额）
        try:
            frozen_rows = int(banner_rows) + 2  # meta+header + (可选 banner)
            want_rows = max(int(len(rows)), int(frozen_rows) + 1)  # frozen=N -> 最小 N+1
            self._set_sheet_grid_properties(
                api_title,
                row_count=want_rows,
                col_count=3,
        ...
        # 样式：banner + header（避免 API tab 看起来“没对齐/不统一”）
        try:
            sh_id = int(self.sheet_id(api_title))
        except Exception:
            sh_id = None
        if sh_id is not None:
            reqs: list[dict[str, Any]] = []
            ...
            if int(banner_rows) > 0:
                ...
                reqs.append(
                    {
                        "repeatCell": {
                            "range": {"sheetId": int(sh_id), "startRowIndex": 0, "endRowIndex": 1, "startColumnIndex": 0, "endColumnIndex": 3},
                            "cell": {
                                "userEnteredFormat": {
                                    "backgroundColor": _rgb(1.0, 0.97, 0.86),
                                    "textFormat": {"bold": True},
            ...
            # meta 行：紧随 banner（或第 1 行）
            reqs.append(
                {
                    "repeatCell": {
                        "range": {"sheetId": int(sh_id), "startRowIndex": int(meta_r0), "endRowIndex": int(meta_r0) + 1, "startColumnIndex": 0, "endColumnIndex": 3},
                        "cell": {
                            "userEnteredFormat": {
                                "backgroundColor": _rgb(0.96, 0.97, 0.98),
                                "textFormat": {"bold": True},
            ...
            reqs.append(
                {
                    "updateDimensionProperties": {
                        "range": {"sheetId": int(sh_id), "dimension": "ROWS", "startIndex": int(meta_r0), "endIndex": int(meta_r0) + 1},
                        "properties": {"pixelSize": 21},
                        "fields": "pixelSize",
                    }
                }
            ...
            # header 行：紧随 meta（或第 2 行）
            hdr_r0 = int(meta_r0) + 1
            reqs.append(
                {
                    "repeatCell": {
                        "range": {"sheetId": int(sh_id), "startRowIndex": int(hdr_r0), "endRowIndex": int(hdr_r0) + 1, "startColumnIndex": 0, "endColumnIndex": 3},
                        "cell": {
                            "userEnteredFormat": {
                                "backgroundColor": _rgb(0.95, 0.95, 0.95),
                                "textFormat": {"bold": True},
                                "horizontalAlignment": "CENTER",
                                "verticalAlignment": "MIDDLE",
                                "wrapStrategy": "CLIP",
            ...
            # 列宽：A(jsonl) / B(说明) / C(请求命令)
            col_widths = [520, 520, 860]
            for ci, px in enumerate(col_widths[:3]):
                reqs.append(
                    {
                        "updateDimensionProperties": {
                            "range": {
            ...
            # data 行：禁止自动换行（否则 jsonl 会把行高撑爆）
            data_r0 = int(hdr_r0) + 1
            data_r1 = int(len(rows))
            if data_r1 > data_r0:
                reqs.append(
                    {
                        "repeatCell": {
                            "range": {
                                "sheetId": int(sh_id),
                                "startRowIndex": int(data_r0),
                                "endRowIndex": int(data_r1),
                                "startColumnIndex": 0,
                                "endColumnIndex": 3,
                            },
                            "cell": {
                            ...
                            "fields": "userEnteredFormat(textFormat.fontFamily,textFormat.fontSize,horizontalAlignment,verticalAlignment,wrapStrategy)",
                        }
                    }
                )
            ...
            # Zebra 灰白交替（数据行）：提升可读性（白色不变，灰色为淡灰）
            try:
                if data_r1 > data_r0:
                    self.apply_row_banding_zebra(
                        title=api_title,
                        start_row_index=int(data_r0),
                        end_row_index=int(data_r1),
                        start_col_index=0,
                        end_col_index=3,
                        first_band_color=_rgb(1.0, 1.0, 1.0),
                        second_band_color=_rgb(0.97, 0.97, 0.97),
                        clear_existing=True,
                    )
            except Exception:
                pass
    ...
    def prune_tabs(
        ...
        *,
        ...
        # API tab：当启用“公开 API 目录表”时，minimal schema 的 prune 不能把该 tab 删掉。
        # - env: SHEETS_API_ENABLE=1
        # - env: SHEETS_API_TAB_TITLE（默认 API）
        # - env: SHEETS_PRUNE_KEEP_API_TAB=0 可强制不保留（极少需要）
        api_title = (os.environ.get("SHEETS_API_TAB_TITLE", "API") or "API").strip() or "API"
        api_enable_raw = (os.environ.get("SHEETS_API_ENABLE", "0") or "0").strip().lower()
        api_enabled = api_enable_raw not in {"0", "off", "false", "no", ""}
        keep_api_tab = api_enabled and (os.environ.get("SHEETS_PRUNE_KEEP_API_TAB", "1") or "1").strip() != "0"
        ...
        if minimal_throttle:
            ...
            keep_news = (os.environ.get("SHEETS_PRUNE_KEEP_NEWS_TAB", "1") or "1").strip() != "0"
            tab_news = _env_text("SHEETS_TAB_NEWS", "实时新闻")
            keep_news_filter = (os.environ.get("SHEETS_PRUNE_KEEP_NEWS_FILTER_TAB", "1") or "1").strip() != "0"
            tab_news_filter = _env_text("SHEETS_TAB_NEWS_FILTER", "新闻过滤配置")
            ...
            if keep_symbol_tabs is not None:
                parts: list[str] = [
                    ...
                    f"keep_api={int(keep_api_tab)}:{api_title}",
                    ...
                    f"keep_news={int(keep_news)}:{tab_news}",
                    f"keep_news_filter={int(keep_news_filter)}:{tab_news_filter}",
        ...
        # 宏观展示页（单工作簿）：一旦用户创建这些 tab，minimal schema 的 prune 必须保留它们，
        # 否则会出现“刚建出来立刻被后台 prune 删掉”的体验灾难。
        macro_tabs = {
            "宏观大宗看板",
            "01_DASHBOARD",
            "10_SNAPSHOT_WIDE",
        ...
        for t in macro_tabs:
            if t in self._sheet_id_by_title:
                keep.add(t)

        # 外部数据旁路：实时新闻（高频刷新）。默认保留，否则会被 minimal prune 误删。
        keep_news = (os.environ.get("SHEETS_PRUNE_KEEP_NEWS_TAB", "1") or "1").strip() != "0"
        if keep_news:
            keep.add(_env_text("SHEETS_TAB_NEWS", "实时新闻"))

        # 外部数据旁路：新闻过滤配置（在线可编辑黑名单/删词规则）。默认保留，否则会被 minimal prune 误删。
        keep_news_filter = (os.environ.get("SHEETS_PRUNE_KEEP_NEWS_FILTER_TAB", "1") or "1").strip() != "0"
        tab_news_filter = _env_text("SHEETS_TAB_NEWS_FILTER", "新闻过滤配置")
        if keep_news_filter and tab_news_filter in self._sheet_id_by_title:
            keep.add(tab_news_filter)
