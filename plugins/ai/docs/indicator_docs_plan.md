...
## 目标
- 为 PG 指标表（`indicator_snapshots.*` 内全部表）补全 what/why/how + fields 说明，写入 `src/utils/data_docs.py` 的 `DATA_DOCS`。
- 说明要基于真实字段与计算逻辑，不可凭空编造。

## 输入资产
- 表结构：`assets/database/db/schema/021_tg_cards_sqlite_parity.sql`（建表 DDL）
- 计算逻辑：
  - 指标注册：`plugins/trading/src/indicators/__init__.py`
  - 指标实现：`plugins/trading/src/indicators/batch/*.py`、`plugins/trading/src/indicators/incremental/*.py`
  - 相关基座：`plugins/trading/src/indicators/base.py`、`plugins/trading/src/core/`
...
## 任务拆解
1) **枚举表 + 字段**
   - 用 PG `information_schema.columns` 列出全部表、字段、类型（schema=`indicator_snapshots`）。
...
6) **验证**
   - 运行轻量校验脚本：遍历 `indicator_snapshots` 表名，确认 `DATA_DOCS` 覆盖率 100%；输出缺失列表。
