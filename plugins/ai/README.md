...
> 标签：研究型插件 / 架构例外
> 当前 `plugins/ai` 允许只读直连 `facts_data` 与 `derived_data`，因为它是离线分析/高载荷研究插件，不是对外统一读出口。统一读出口仍然只有 `core/query`。
...
Telegram 用户 → bot/handler.py (币种/周期选择)
             → pipeline.py
             → data/fetcher.py (TimescaleDB + PG(indicator_snapshots))
             → prompt/builder.py
...
- 允许：只读 `facts_data.market.*`、`derived_data.indicator_snapshots.*`
- 禁止：写数据库、对外暴露成新的 HTTP 查询服务、复制这套例外到 telegram/sheets/vis
- 如未来演化成常驻消费服务，应优先收口到 `core/query`
