...
直接从 PostgreSQL 读取 K 线与期货指标快照主表
...
try:
    from ..config import (
        ...
        get_database_url,
        get_market_candles_table,
        get_market_futures_metrics_table,
    )
...
try:
    ...
    emit_lineage_event = None  # type: ignore

logger = logging.getLogger(__name__)

LINEAGE_ACTOR_ID = "plugins/signal"
LINEAGE_RESOURCE_COOLDOWN = "derived/signal_runtime/cooldown"
LINEAGE_RESOURCE_HISTORY = "derived/signal_runtime/signal_history"
...
class PGSignalEngine(BaseEngine):
    ...
    def __init__(self, db_url: str = None, symbols: list[str] = None):
        ...
        # lineage（派生写入侧）：聚合记录到 derived_data.governance.lineage_events
        self._lineage_run_id = (os.environ.get("LINEAGE_RUN_ID") or "").strip() or f"signal_{datetime.utcnow().strftime('%Y%m%dT%H%M%S%fZ')}_{os.getpid()}"

    def _emit_lineage(self, *, resource_id: str, status: str, meta: dict) -> None:
        """写入血缘事件（fail-soft）。"""
        if emit_lineage_event is None:
            return
        dsn = (getattr(self._cooldown_storage, "database_url", "") or "").strip()
        if not dsn:
            return
        try:
            emit_lineage_event(
                ...
                run_id=str(getattr(self, "_lineage_run_id", "")),
                action="produce",
    ...
    def _fetch_latest_candles(self) -> dict[str, dict]:
        ...
        result = {}
        candles_table = get_market_candles_table("1m")
        try:
            query = """
                WITH ranked AS (
    ...
    def _fetch_latest_metrics(self) -> dict[str, dict]:
        ...
        result = {}
        metrics_table = get_market_futures_metrics_table("5m")
        try:
            query = """
                WITH ranked AS (
        ...
        while self._running:
            try:
                ...
                if signals:
                    ...
                    meta = {
                        "item_count": int(len(signals)),
                        "symbols": int(len(self.symbols)),
                        "persistence_failures": int(getattr(self, "persistence_failures", 0) or 0),
                    }
                    self._emit_lineage(resource_id=LINEAGE_RESOURCE_COOLDOWN, status="success", meta=meta)
                    self._emit_lineage(resource_id=LINEAGE_RESOURCE_HISTORY, status="success", meta=meta)
            except Exception as e:
                ...
                self._emit_lineage(resource_id=LINEAGE_RESOURCE_COOLDOWN, status="failed", meta=meta)
                self._emit_lineage(resource_id=LINEAGE_RESOURCE_HISTORY, status="failed", meta=meta)
            time.sleep(interval)
