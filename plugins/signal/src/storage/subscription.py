...
class PgSubscriptionManager:
    """PG 订阅管理器（signal_runtime.signal_subs）"""
    ...
    def __init__(self, database_url: str | None = None, *, schema: str | None = None):
        ...
        self.table = get_signal_table_name("subs")
    ...
    def _load(self, user_id: int) -> dict | None:
        try:
            with self._conn() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        f"SELECT enabled, tables FROM {self.schema}.{self.table} WHERE user_id=%s",
                        (int(user_id),),
                    )
                    row = cur.fetchone()
            if row:
        ...
        try:
            ...
            with self._conn() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        f"""
                        INSERT INTO {self.schema}.{self.table} (user_id, enabled, tables)
                        VALUES (%s, %s, (%s)::jsonb)
    ...
    def get_enabled_subscribers(self) -> list[int]:
        try:
            with self._conn() as conn:
                with conn.cursor() as cur:
                    cur.execute(f"SELECT user_id FROM {self.schema}.{self.table} WHERE enabled=true")
