...
def main():
    ...
    if args.test:
        from config import get_database_url, get_derived_database_url
        from rules import RULE_COUNT, TABLE_COUNT

        logger.info("=== Signal Service 配置测试 ===")
        logger.info(f"  FACTS URL: {get_database_url()[:50]}...")
        logger.info(f"  DERIVED URL: {get_derived_database_url()[:50]}...")
