...
// 统一加载 TradeCat 环境变量（优先 assets/config/.env；兼容旧的 config/.env）
const findRepoRoot = () => {
    // 从当前目录向上找，兼容：
    // 1) 单服务部署（tradecat/config/.env）
    // 2) 单仓库部署（tradecat/assets/config/.env）
    let dir = path.resolve(__dirname);
    for (let i = 0; i < 10; i += 1) {
        const hasAssetsEnv = fs.existsSync(path.join(dir, 'assets', 'config', '.env'));
        const hasAssetsExample = fs.existsSync(path.join(dir, 'assets', 'config', '.env.example'));
        const hasConfigEnv = fs.existsSync(path.join(dir, 'config', '.env'));
        if (hasAssetsEnv || hasAssetsExample || hasConfigEnv) return dir;
        const parent = path.dirname(dir);
        if (parent === dir) break;
        dir = parent;
    }
    // fallback：保持可用（不保证能找到 env）
    return path.resolve(__dirname, '../../../../../../');
};

const repoRoot = findRepoRoot();
const dotenvCandidates = [
    path.join(repoRoot, 'assets', 'config', '.env'),
    path.join(repoRoot, 'config', '.env'),
];
dotenvCandidates.forEach((p) => {
