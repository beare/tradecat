...
# 3) Start core services (ai + signal + query + telegram + trading)
./scripts/start.sh start
./scripts/start.sh status
```

> Note: top-level `./scripts/start.sh` manages `plugins/ai`, `plugins/signal`, `core/query`, `plugins/telegram`, `plugins/trading` by default (`plugins/ai` is a sub-module; readiness check only, no standalone process; and `core/query` starts before telegram/sheets; legacy alias: `api-service`).
> **Important**: since 2026-03, the consumption layer (Telegram/Sheets/visualization) is no longer allowed to read DB directly. All reads go through **Query Service** (`core/query`, `/api/v1`). Do **not** stop `core/query` while running Telegram/Sheets.
> Exception: `plugins/ai` is currently treated as a research / offline analysis plugin and is allowed to read `facts_data` / `derived_data` directly in read-only mode; it is not a new public read gateway.
> Binance data stack: `core/market/binance/` (LF/HF are runtime groups, not child projects; not enabled by default).
> Optional services (manual start):
> - `cd plugins/sheets && ./scripts/start.sh start` (Google Sheets dashboard sync, daemon by default)
...
- Database layout (repo examples are written for this):
  - `FACTS_DATABASE_URL=postgresql://.../facts_data`
  - `DERIVED_DATABASE_URL=postgresql://.../derived_data`
  - legacy `DATABASE_URL` still points to `facts_data`
  - If you customize ports (or split LF/HF), update scripts and examples consistently.
- Key fields:
	  - `FACTS_DATABASE_URL` (facts DB; default `localhost:5433/facts_data`)
	  - `DERIVED_DATABASE_URL` (derived DB; default `localhost:5433/derived_data`)
	  - `DATABASE_URL` (legacy compatibility DSN; defaults to `facts_data`)
	  - `assets/contracts/resources.v1.yaml` / `probes.v1.yaml` / `bindings.v1.yaml` are now the single source of truth for semantic resources and canonical object naming
	  - `assets/catalog/*` is export-only projection; runtime must not decide central-covered object names from `.env` or legacy catalog
	  - Runtime code should stop hardcoding `market.*` / `alternative.*` / `indicator_snapshots.*` / `signal_runtime.*`; use central contracts through `assets/common/contracts/db_contracts.py`. `.env.example` now keeps only connection flags and a small set of non-central dynamic object names
	  ...
	  - `QUERY_MARKET_TABLE_EXISTS_TTL_SEC` (optional: `facts_data.market.*` table-existence TTL cache seconds; default 30)
...
cd core/market/binance && .venv/bin/pip install pandas psycopg2-binary huggingface_hub
...
for f in assets/database/db/schema/*.sql; do
  psql -h localhost -p 5433 -U postgres -d facts_data -f "$f"
done

# 1. Import candlesticks (373M)
zstd -d candles_1m.bin.zst -c | psql -h localhost -p 5433 -U postgres -d facts_data \
  -c "COPY market.binance_futures_um_candles_1m FROM STDIN WITH (FORMAT binary)"

# 2. Import futures metrics (94M)
zstd -d futures_metrics_5m.bin.zst -c | psql -h localhost -p 5433 -U postgres -d facts_data \
  -c "COPY market.binance_futures_um_metrics_snapshot_5m FROM STDIN WITH (FORMAT binary)"
```
...
- **Port selection**: `assets/config/.env.example` defaults to a single port 5433; keep scripts and services consistent if you change ports (or split LF/HF).
...
./scripts/init.sh hf  # legacy alias: hf-service
```
...
graph TD
    ...
    subgraph DS["📦 Binance Stack (LF runtime group)<br><small>Python, asyncio, ccxt, cryptofeed</small>"]
    ...
    PG_IND[("🗄️ indicator_snapshots.*<br>PostgreSQL Indicator Store")]
    TR_IND --> PG_IND
...
|:---|:---:|:---|:---|
| **binance-stack (LF)** | - | Crypto candlestick collection, futures metrics, historical backfill | Python, asyncio, ccxt, cryptofeed |
...
| **predict-service** | - | Prediction market signals (Polymarket/Kalshi/Opinion, now under consumption) | Node.js + Python utilities |
...
| **TimescaleDB (single instance)** | 5433 | LF+HF (candles/indicators + atomic facts; `DATABASE_URL`/`BINANCE_HF_DATABASE_URL` default to the same instance/port) | PostgreSQL 16 + TimescaleDB |
...
graph LR
    subgraph Collection
        A["🌐 Binance WebSocket"] --> B["📦 Binance Stack (LF)"]
    end
    ...
    subgraph Calculation
        ...
        D --> E2[("🗄️ indicator_snapshots.*<br>PostgreSQL Indicator Store")]
    end
...
# 2. Restore schema
zstd -d schema.sql.zst -c | psql -h localhost -p 5433 -U postgres -d facts_data
...
│   ├── 📂 archive/                 # Archived legacy sources/docs (not on the main chain)
...
│
├── 📂 core/                        # Fact layer (collection/storage/query)
│   ├── 📂 market/                  # Market facts (writes TimescaleDB)
│   │   └── 📂 binance/             # Source: Binance
│   │       ├── 📂 data/            # Legacy low-frequency ingestion (opt-in)
│   │       └── 📂 hf/  # Binance official archive-aligned ingestion (HF atomic facts)
│   │
│   ├── 📂 alternative/             # Alternative facts (events/news/...)
│   │   ├── 📂 api/
│   │   ├── 📂 discord/
│   │   ├── 📂 hyper/
│   │   ├── 📂 investing/
│   │   ├── 📂 news/
│   │   ├── 📂 telegram/            # TG fact ingestion only (legacy helpers archived)
│   │   └── 📂 x/
│   │
│   ├── 📂 query/                   # Single reader (/api/v1)
│   │
│   │
│   └── 📂 storage/                 # DDL/migrations/retention (reserved)
│
├── 📂 plugins/                     # Plugins (reads core/query; see plugins/README.md for categorization)
│   ├── 📂 trading/                 # Derived: indicators (writes indicator_snapshots.*)
│   ├── 📂 signal/                  # Derived: signals (rules engine)
│   ├── 📂 ai/                      # AI: readiness check / offline analysis entry (no daemon)
│   ├── 📂 telegram/                # Export: Telegram Bot + event/signal publishers
│   ├── 📂 sheets/                  # Export: Google Sheets dashboard sync (optional)
│   ├── 📂 llm-gateway/             # Gateway: OpenAI-compatible (nofx integration)
│   ├── 📂 predict/                 # Other: prediction markets
│   ├── 📂 fate/                    # Other: fate/astrology
│   ├── 📂 vis/                     # Other: visualization (optional)
│   └── 📂 nofx-dev/                # Other: external mirror (not core chain)
│
...
```bash
# Binance Stack (LF runtime group; legacy alias: data-service)
...
cd plugins/trading  # or plugins/telegram
...
```bash
# Binance Stack (LF) logs
tail -f core/market/binance/var/lf/logs/backfill.log
tail -f core/market/binance/var/lf/logs/metrics.log
tail -f core/market/binance/var/lf/logs/ws.log

# trading-service logs
tail -f plugins/trading/logs/service.log

# telegram-service logs
tail -f plugins/telegram/logs/bot.log

# signal-service logs
tail -f plugins/signal/logs/signal-service.log
...
ps aux | grep -E "binance\.runtime\.lf_|trading-service|telegram|simple_scheduler"
...
# Common queries
-- Candlestick count
SELECT COUNT(*) FROM market.binance_futures_um_candles_1m;

-- Latest data time
SELECT MAX(bucket_ts) FROM market.binance_futures_um_candles_1m;

-- Token list
SELECT DISTINCT symbol FROM market.binance_futures_um_candles_1m ORDER BY symbol;

-- Single token data
SELECT * FROM market.binance_futures_um_candles_1m
...
<details>
<summary><strong>Expand👉 Indicator Store (PG: indicator_snapshots) Queries</strong></summary>

```bash
# Initialize DDL (once; creates indicator_snapshots schema + 38 tables)
...
# List tables
PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d derived_data \
    -c "\\dt indicator_snapshots.*"

# Sample query (table/column names contain Chinese and .py; must be double-quoted)
PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d derived_data \
    -c "SELECT \\\"交易对\\\",\\\"周期\\\",\\\"数据时间\\\",\\\"ATR百分比\\\" FROM indicator_snapshots.\\\"ATR波幅扫描器.py\\\" WHERE \\\"周期\\\"='15m' ORDER BY \\\"数据时间\\\" DESC LIMIT 5;"
```
...
> Note: the indicator store is now PostgreSQL-only (`indicator_snapshots.*`).
> If you still keep a historical SQLite indicator store, use it only for migration/replay checks, not as the current runtime source of truth.
...
# Restore schema
zstd -d schema_*.sql.zst -c | psql -h localhost -p 5433 -U postgres -d facts_data

# Restore candlestick data
zstd -d candles_1m_*.bin.zst -c | psql -h localhost -p 5433 -U postgres -d facts_data \
    -c "COPY market.binance_futures_um_candles_1m FROM STDIN WITH (FORMAT binary)"

# Restore futures data
zstd -d futures_metrics_*.bin.zst -c | psql -h localhost -p 5433 -U postgres -d facts_data \
    -c "COPY market.binance_futures_um_metrics_snapshot_5m FROM STDIN WITH (FORMAT binary)"
```
